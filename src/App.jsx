...
const uploadImageToCloudinary = async (file) => {
  ...
  if (!file || !allowedTypes.includes(file.type)) throw new Error("Povolené jsou pouze JPG, PNG a WebP.");
  if (file.size > maxBytes) throw new Error("Soubor může mít maximálně 10 MB.");
...
const getBuyerData = (listingId) => {
  try { return JSON.parse(sessionStorage.getItem("vebre_buyer_data_" + listingId) || "null"); }
  catch { return null; }
};
const saveBuyerData = (listingId, data) => {
  // Citlivé údaje zůstávají pouze v aktuální kartě prohlížeče. Produkce musí použít šifrované serverové úložiště.
  sessionStorage.setItem("vebre_buyer_data_" + listingId, JSON.stringify(data));
};
...
const D = { fontFamily: "'DM Serif Display', Georgia, serif", letterSpacing: "-0.025em" };

// ── DESIGN SYSTEM ────────────────────────────────────────────────────────────
if (typeof document !== 'undefined' && !document.getElementById('vebre-fonts')) {
  ...
  s.innerHTML = `
    * { box-sizing: border-box; }
    body { background: #FAFAF9; margin: 0; font-family: 'Inter', sans-serif; -webkit-font-smoothing: antialiased; color: #0F0F0F; }
    ::selection { background: #1B4FD8; color: white; }
    ::-webkit-scrollbar { width: 6px; }
    ::-webkit-scrollbar-track { background: #F0F4FF; }
    ::-webkit-scrollbar-thumb { background: #CBD5F0; border-radius: 99px; }
    ...
    :root {
      ...
      --radius-md: 16px;
      --radius-lg: 24px;
      --shadow-sm: 0 1px 2px rgba(19,35,29,.04), 0 8px 24px rgba(19,35,29,.05);
      --shadow-md: 0 18px 55px rgba(19,35,29,.10);
    }
    html { scroll-behavior: smooth; }
    body { background: var(--canvas); color: var(--ink); }
    button, input, select, textarea { font: inherit; }
    button:focus-visible, input:focus-visible, select:focus-visible, textarea:focus-visible, [tabindex]:focus-visible {
      outline: 3px solid rgba(22,92,70,.22) !important;
      outline-offset: 2px;
    }
    ...
    @media (max-width: 600px) {
      [style*="padding: 0px 32px"], [style*="padding: 0 32px"] { padding-inline: 16px !important; }
      [style*="padding: 48px"], [style*="padding: 52px 32px 40px"] { padding: 28px 18px !important; }
...
const Badge = ({ children, color = "stone" }) => {
  ...
  return <span style={{ background: bg, color: tx, fontSize: 11, fontWeight: 600, padding: "3px 11px", borderRadius: 99, letterSpacing: "0.03em", whiteSpace: "nowrap", fontFamily: "'Inter', sans-serif" }}>{children}</span>;
};

const Btn = ({ children, onClick, variant = "dark", small, full, disabled, type = "button", ariaLabel }) => {
  const v = {
    dark: { background: "var(--brand)", color: "#fff", border: "1px solid var(--brand)", shadow: "0 8px 20px rgba(22,92,70,.18)" },
    blue: { background: "var(--brand)", color: "#fff", border: "1px solid var(--brand)", shadow: "0 8px 20px rgba(22,92,70,.18)" },
    outline: { background: "#fff", color: "var(--ink)", border: "1px solid var(--line)", shadow: "none" },
    ghost: { background: "transparent", color: "var(--muted)", border: "1px solid transparent", shadow: "none" },
    gold: { background: "var(--accent)", color: "#fff", border: "1px solid var(--accent)", shadow: "0 8px 20px rgba(183,137,80,.18)" },
  };
  const s = v[variant] || v.dark;
  return (
    <button type={type} aria-label={ariaLabel} onClick={onClick} disabled={disabled} style={{
      background: s.background, color: s.color, border: s.border, boxShadow: s.shadow,
      borderRadius: 12, padding: small ? "9px 15px" : "12px 22px",
      minHeight: small ? 38 : 44, fontSize: small ? 13 : 14, fontWeight: 650,
...
const Card = ({ children, style, onClick }) => (
  <div onClick={onClick} role={onClick ? "button" : undefined} tabIndex={onClick ? 0 : undefined}
    ...
    style={{ background: "var(--surface)", borderRadius: "var(--radius-md)", border: "1px solid var(--line)",
      overflow: "hidden", boxShadow: "var(--shadow-sm)", cursor: onClick ? "pointer" : undefined,
...
const AIBadge = () => (
  <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: "linear-gradient(135deg, #1B4FD8 0%, #1E3A8A 100%)", color: "#fff", fontSize: 11, fontWeight: 700, padding: "4px 10px", borderRadius: 99, letterSpacing: "0.05em", boxShadow: "0 2px 6px rgba(27,79,216,0.3)" }}>✦ AI</span>
);

const Input = ({ label, value, onChange, type = "text", placeholder, autoComplete = "off", inputMode }) => {
  const id = `field-${String(label).toLowerCase().replace(/[^a-z0-9]+/g, "-")}`;
  return (
    <div style={{ marginBottom: 16 }}>
      <label htmlFor={id} style={{ fontSize: 12, fontWeight: 650, color: "var(--muted)", display: "block", marginBottom: 7 }}>{label}</label>
      <input id={id} type={type} value={value} onChange={onChange} placeholder={placeholder}
        autoComplete={autoComplete} inputMode={inputMode}
        style={{ width: "100%", minHeight: 46, padding: "12px 14px", borderRadius: 12, border: "1px solid var(--line)",
          fontSize: 14, fontFamily: "'Inter', sans-serif", boxSizing: "border-box", color: "var(--ink)",
          background: "#fff", outline: "none", transition: "border .15s, box-shadow .15s" }}
        onFocus={e => { e.target.style.borderColor = "var(--brand)"; e.target.style.boxShadow = "0 0 0 4px rgba(22,92,70,.08)"; }}
...
const ContractView = ({ listing, buyer, paid }) => {
  ...
  const printContract = () => {
    ...
    <p style="text-align:center;font-size:11px;color:#999;margin-top:40px">Trustera · trustera.cz · ${today}</p>
    </body></html>`);
    win.document.close();
    setTimeout(() => win.print(), 500);
  };
  return (
    <div style={{ background: "#fff", border: "1px solid #E2E8F4", borderRadius: 16, padding: 28 }}>
      <div style={{ textAlign: "center", marginBottom: 22, paddingBottom: 18, borderBottom: "2px solid #0F0F0F" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
        ...
        <div style={{ background: paid ? "#f0fdf4" : "#EFF6FF", borderRadius: 12, padding: 14 }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: "#999", marginBottom: 8 }}>PRODÁVAJÍCÍ</div>
          {paid ? (<><div style={{ fontWeight: 700, fontSize: 14, color: "#059669" }}>{listing.seller.name}</div><div style={{ fontSize: 12, color: "#059669" }}>📧 {listing.seller.email}</div><div style={{ fontSize: 12, color: "#059669" }}>📞 {listing.seller.phone}</div></>) : (<><div style={{ fontWeight: 700, fontSize: 14 }}>{listing.seller.anonymousId}</div><div style={{ fontSize: 11, color: "#1E3A8A", marginTop: 6 }}>🔒 Odhaleno po zaplacení</div></>)}
        </div>
      </div>
      <div style={{ background: "#F0F4FF", borderRadius: 12, padding: 14, marginBottom: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
        <div style={{ border: "1.5px solid #E2E8F4", borderRadius: 12, padding: 14 }}>
        ...
        <div style={{ border: "1.5px solid #1B4FD8", borderRadius: 12, padding: 14, background: "#EEF2FF" }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: "#1E3A8A", marginBottom: 4 }}>REZERVAČNÍ POPLATEK</div>
          <div style={{ ...D, fontSize: 18, fontWeight: 800, color: "#1B4FD8" }}>{fmtFull(listing.reservationFee)}</div>
        </div>
      </div>
      <div style={{ borderTop: "1px solid #EEF2FB", paddingTop: 16, display: "flex", gap: 10, alignItems: "center" }}>
        <button onClick={printContract} style={{ display: "flex", alignItems: "center", gap: 8, background: "#0F0F0F", color: "#fff", border: "none", borderRadius: 10, padding: "10px 20px", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>📄 Stáhnout jako PDF</button>
...
const InsolvencyCheck = ({ firstName, lastName, onResult }) => {
  ...
  if (status === "idle") return <button onClick={check} style={{ display: "flex", alignItems: "center", gap: 8, background: "#F0F4FF", border: "1.5px solid #E2E8F4", borderRadius: 10, padding: "10px 16px", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>🔍 Zkontrolovat insolvenci</button>;
  if (status === "loading") return <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 16px", background: "#F0F4FF", borderRadius: 10, fontSize: 13, color: "#888" }}><div style={{ width: 16, height: 16, border: "2px solid #1B4FD8", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />Ověřuji…</div>;
  ...
  return <div style={{ background: "#EFF6FF", border: "1px solid #BFDBFE", borderRadius: 10, padding: "12px 16px", fontSize: 13, color: "#1E3A8A" }}><div style={{ fontWeight: 700, marginBottom: 6 }}>⚠️ Automatická kontrola nedostupná</div><a href="https://isir.justice.cz/isir/ueu/evidence_upadcu.do" target="_blank" rel="noreferrer" style={{ background: "#1E3A8A", color: "#fff", padding: "6px 14px", borderRadius: 7, fontSize: 12, fontWeight: 700, textDecoration: "none", display: "inline-block" }}>Zkontrolovat ručně →</a></div>;
};
      ...
      return (
        <div key={s} style={{ display: "flex", alignItems: "center", flex: i < steps.length - 1 ? 1 : undefined }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ width: 28, height: 28, borderRadius: "50%", background: done ? "#059669" : active ? "#0F0F0F" : "#E2E8F4", color: done || active ? "#fff" : "#999", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, flexShrink: 0 }}>{done ? "✓" : i + 1}</div>
            <span style={{ fontSize: 13, fontWeight: active ? 700 : 400, color: active ? "#0F0F0F" : done ? "#059669" : "#999", whiteSpace: "nowrap" }}>{s}</span>
          </div>
          {i < steps.length - 1 && <div style={{ flex: 1, height: 1, background: done ? "#059669" : "#E2E8F4", margin: "0 12px" }} />}
        </div>
      );
    })}
  </div>
...
const ReservePage = ({ id, setPage, user }) => {
  ...
  return (
    <div style={{ maxWidth: 760, margin: "40px auto", padding: "0 24px" }}>
      <button onClick={() => step === "Osobní údaje" ? setPage("listing-" + l.id) : setStep(STEPS[STEPS.indexOf(step) - 1])} style={{ background: "none", border: "none", cursor: "pointer", color: "#888", fontSize: 14, marginBottom: 26, fontFamily: "'Inter', sans-serif" }}>← Zpět</button>
      ...
      {step === "Osobní údaje" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 260px", gap: 24 }}>
          <Card style={{ padding: 24 }}>
            ...
            <div style={{ margin: "4px 0 16px", padding: "14px 16px", background: viewingDone ? "#f0fdf4" : "#F0F4FF", borderRadius: 12, border: "1.5px solid " + (viewingDone ? "#6ee7b7" : "#E2E8F4"), cursor: "pointer" }} onClick={() => setViewingDone(v => !v)}>
          ...
          <Card style={{ padding: 20, alignSelf: "start" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, fontWeight: 800, borderTop: "1px solid #EEF2FB", paddingTop: 12 }}><span>Poplatek</span><span style={{ ...D }}>50 000 Kč</span></div>
          </Card>
        </div>
      )}
      {step === "Smlouva" && (
        <div>
          ...
          <div style={{ marginTop: 20, padding: "16px 20px", background: contractRead ? "#f0fdf4" : "#F0F4FF", borderRadius: 12, border: "1.5px solid " + (contractRead ? "#6ee7b7" : "#E2E8F4"), cursor: "pointer", marginBottom: 20 }} onClick={() => setContractRead(v => !v)}>
      ...
      {step === "Platba" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 260px", gap: 24 }}>
          <Card style={{ padding: 26 }}>
            ...
            <div style={{ background: "#F0F4FF", borderRadius: 12, padding: 18, marginBottom: 18 }}>
            ...
            <div style={{ background: "#EFF6FF", border: "1px solid #BFDBFE", borderRadius: 10, padding: "12px 14px", marginBottom: 18, fontSize: 12, color: "#1E3A8A" }}>⚠️ Po odeslání platby klikněte níže. Smlouva nabude účinnosti a identita prodávajícího bude odhalena.</div>
          ...
          <Card style={{ padding: 20, alignSelf: "start" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, fontWeight: 800, borderTop: "1px solid #EEF2FB", paddingTop: 12 }}><span>K úhradě</span><span style={{ ...D, color: "#1B4FD8" }}>50 000 Kč</span></div>
          </Card>
        </div>
      )}
      <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
...
const BookingPage = ({ id, setPage, user }) => {
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          {[{ l: "DATUM", v: selectedSlot.date }, { l: "ČAS", v: selectedSlot.time }, { l: "KONTAKT", v: form.phone }, { l: "STATUS", v: "⏳ Čeká na potvrzení" }].map(item => (
            <div key={item.l} style={{ background: "#fff", borderRadius: 10, padding: "10px 14px" }}>
              ...
              <div style={{ fontWeight: 700, fontSize: 13, color: item.l === "STATUS" ? "#d97706" : "#0F0F0F" }}>{item.v}</div>
            </div>
          ))}
        </div>
      </div>
      {/* #22 — SMS + Kalendář */}
      <div style={{ background: "#fff", border: "1px solid #E8EDF8", borderRadius: 14, padding: "16px 20px", marginBottom: 24, textAlign: "left" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12, padding: "10px 14px", background: "#F0F4FF", borderRadius: 10 }}>
          <span style={{ fontSize: 18 }}>📱</span>
          <div style={{ fontSize: 13, color: "#444" }}>
            <span style={{ fontWeight: 600 }}>SMS potvrzení odesláno</span> na <span style={{ fontWeight: 700 }}>{form.phone}</span>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", background: "#F0F4FF", borderRadius: 10 }}>
  ...
  return (
    <div style={{ maxWidth: 900, margin: "32px auto", padding: "0 24px" }}>
      <button onClick={() => setPage("listing-" + l.id)} style={{ background: "none", border: "none", cursor: "pointer", color: "#888", fontSize: 14, marginBottom: 24, fontFamily: "'Inter', sans-serif" }}>← Zpět na inzerát</button>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 24 }}>
        ...
        <div>
          {/* #21 — Typy prohlídek */}
          <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #E8EDF8", padding: 22, marginBottom: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 14 }}>Typ prohlídky</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <div onClick={() => setViewingType("guided")} style={{ padding: 16, borderRadius: 14, border: "2px solid " + (viewingType === "guided" ? "#0F0F0F" : "#E2E8F4"), background: viewingType === "guided" ? "#F0F4FF" : "#fff", cursor: "pointer" }}>
              ...
              <div style={{ padding: 16, borderRadius: 14, border: "2px solid #E2E8F4", background: "#F0F4FF", opacity: 0.6, position: "relative" }}>
                <div style={{ fontSize: 26, marginBottom: 8 }}>🔑</div>
                <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 4, display: "flex", alignItems: "center", gap: 6 }}>
                  Samostatná
                  <span style={{ fontSize: 9, fontWeight: 700, background: "#E2E8F4", color: "#888", padding: "2px 7px", borderRadius: 99, letterSpacing: "0.05em" }}>BETA</span>
                </div>
          ...
          <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #E8EDF8", padding: 24, marginBottom: 16 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
              <h3 style={{ fontWeight: 700, fontSize: 15, margin: 0 }}>📅 Vyberte termín</h3>
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => setActiveWeek(w => Math.max(0, w - 1))} disabled={activeWeek === 0} style={{ width: 32, height: 32, borderRadius: 8, border: "1.5px solid #E2E8F4", background: "#fff", cursor: activeWeek === 0 ? "not-allowed" : "pointer", opacity: activeWeek === 0 ? 0.4 : 1, fontSize: 16 }}>←</button>
                <button onClick={() => setActiveWeek(w => Math.min(weeks.length - 1, w + 1))} disabled={activeWeek === weeks.length - 1} style={{ width: 32, height: 32, borderRadius: 8, border: "1.5px solid #E2E8F4", background: "#fff", cursor: activeWeek === weeks.length - 1 ? "not-allowed" : "pointer", opacity: activeWeek === weeks.length - 1 ? 0.4 : 1, fontSize: 16 }}>→</button>
              </div>
            </div>
                      ...
                      {day.slots.map(slot => {
                        ...
                        return (
                          <button key={slot.id} onClick={() => setSelectedSlot(slot)}
                            style={{ padding: "8px 16px", borderRadius: 10, border: "1.5px solid " + (selected ? "#0F0F0F" : "#E2E8F4"), background: selected ? "#0F0F0F" : "#fff", color: selected ? "#fff" : "#0F0F0F", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif", transition: "all 0.15s" }}>
                            {slot.time}
                          </button>
                        );
                      })}
          ...
          {/* Kontaktní formulář */}
          {selectedSlot && (
            <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #E8EDF8", padding: 24 }}>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <textarea value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} rows={2} placeholder="Např. přijdu s partnerem, potřebuji zkontrolovat sklep…" style={{ width: "100%", padding: "11px 13px", borderRadius: 11, border: "1.5px solid #E2E8F4", fontSize: 13, fontFamily: "'Inter', sans-serif", resize: "none", boxSizing: "border-box" }} />
              </div>
        ...
        {/* Sidebar */}
        <div>
          <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #E8EDF8", padding: 22, marginBottom: 14 }}>
            ...
            {selectedSlot ? (
              <div>
                ...
                <button onClick={() => setSelectedSlot(null)} style={{ background: "none", border: "none", color: "#999", fontSize: 12, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>✕ Zrušit výběr</button>
              </div>
            ) : (
          ...
          <div style={{ background: "#EFF6FF", border: "1px solid #BFDBFE", borderRadius: 14, padding: 16, fontSize: 12, color: "#1E3A8A", lineHeight: 1.6 }}>
...
const AvailabilityPage = ({ id, setPage, user }) => {
  ...
  return (
    <div style={{ maxWidth: 860, margin: "32px auto", padding: "0 24px" }}>
      <button onClick={() => setPage("seller-dash")} style={{ background: "none", border: "none", cursor: "pointer", color: "#888", fontSize: 14, marginBottom: 24, fontFamily: "'Inter', sans-serif" }}>← Zpět na dashboard</button>
      ...
      {/* Záložky */}
      <div style={{ display: "flex", gap: 4, marginBottom: 24, borderBottom: "1px solid #E2E8F4" }}>
        {[{ id: "wizard", label: "⚡ Rychlé nastavení" }, { id: "preview", label: "Termíny (" + slots.length + ")" }].map(t => (
          <button key={t.id} onClick={() => setActiveTab(t.id)} style={{ padding: "9px 18px", background: "none", border: "none", borderBottom: activeTab === t.id ? "2px solid #0F0F0F" : "2px solid transparent", color: activeTab === t.id ? "#0F0F0F" : "#888", fontWeight: activeTab === t.id ? 700 : 500, fontSize: 13, cursor: "pointer", fontFamily: "'Inter', sans-serif", marginBottom: -1 }}>{t.label}</button>
        ))}
      </div>

      {activeTab === "wizard" && (
        <div>
          <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #E8EDF8", padding: 26, marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {DAYS.map((d, i) => {
                ...
                return (
                  <button key={d} onClick={() => toggleDay(dayNum)} style={{ padding: "10px 18px", borderRadius: 10, border: "1.5px solid " + (active ? "#0F0F0F" : "#E2E8F4"), background: active ? "#0F0F0F" : "#fff", color: active ? "#fff" : "#555", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>
                    {DAY_NAMES[i]}
                  </button>
                );
              })}
          ...
          <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #E8EDF8", padding: 26, marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {TIME_SLOTS.map(t => {
                ...
                return (
                  <button key={t} onClick={() => toggleTime(t)} style={{ padding: "10px 16px", borderRadius: 10, border: "1.5px solid " + (active ? "#1B4FD8" : "#E2E8F4"), background: active ? "#EEF2FF" : "#fff", color: active ? "#1E3A8A" : "#555", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>
                    {t}
                  </button>
                );
              })}
          ...
          <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #E8EDF8", padding: 26, marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {[1, 2, 3, 4].map(w => (
                <button key={w} onClick={() => setWeeksAhead(w)} style={{ flex: 1, padding: "12px 8px", borderRadius: 10, border: "1.5px solid " + (weeksAhead === w ? "#0F0F0F" : "#E2E8F4"), background: weeksAhead === w ? "#0F0F0F" : "#fff", color: weeksAhead === w ? "#fff" : "#555", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif", textAlign: "center" }}>
          ...
          {slots.length === 0 ? (
            ...
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {Object.entries(byDay).map(([dateKey, day]) => (
                  <div key={dateKey} style={{ background: "#fff", borderRadius: 14, border: "1px solid #E8EDF8", padding: "16px 20px" }}>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                      {day.slots.map(slot => (
                        <div key={slot.id} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 12px", borderRadius: 9, background: "#F0F4FF", border: "1.5px solid #E2E8F4" }}>
...
const TransactionProgress = ({ activeStep = "contract", compact = false }) => {
  ...
  return (
    <div style={{ background: "#fff", borderRadius: compact ? 14 : 20, border: "1px solid #E8EDF8", padding: compact ? "16px 20px" : "22px 28px" }}>
          ...
          return (
            <div key={m.id} style={{ display: "flex", alignItems: "center", flexShrink: 0 }}>
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, minWidth: compact ? 56 : 72 }}>
                <div style={{ width: compact ? 32 : 38, height: compact ? 32 : 38, borderRadius: "50%", background: done ? "#059669" : active ? "#0F0F0F" : "#EEF2FB", border: active ? "3px solid #1B4FD8" : "none", display: "flex", alignItems: "center", justifyContent: "center", fontSize: done ? 13 : 16, transition: "all 0.2s" }}>
                  {done ? <span style={{ color: "#fff", fontWeight: 800 }}>✓</span> : <span style={{ filter: active ? "none" : "grayscale(1)", opacity: active ? 1 : 0.5 }}>{m.icon}</span>}
                </div>
                <div style={{ fontSize: compact ? 9 : 10, fontWeight: active ? 700 : 400, color: done ? "#059669" : active ? "#0F0F0F" : "#bbb", textAlign: "center", lineHeight: 1.3, maxWidth: compact ? 56 : 68 }}>{m.label}</div>
              </div>
              {i < TRANSACTION_MILESTONES.length - 1 && (
                <div style={{ width: compact ? 16 : 24, height: 3, background: done ? "#059669" : "#EEF2FB", borderRadius: 99, margin: "0 2px", marginBottom: compact ? 18 : 22, flexShrink: 0, transition: "background 0.3s" }} />
              )}
            </div>
          );
        })}
...
const BrokerageContractPage = ({ listingData, setPage, onSigned }) => {
  ...
  if (signed) return (
    <div style={{ maxWidth: 560, margin: "60px auto", padding: "0 24px", textAlign: "center" }}>
      ...
      <div style={{ background: "#f0fdf4", border: "1px solid #6ee7b7", borderRadius: 14, padding: 20, marginBottom: 28, textAlign: "left" }}>
        {[["Nemovitost", listingData?.title || "Nový inzerát"], ["Provize Trustera", "2 % z kupní ceny (vč. DPH)"], ["Exkluzivita", "90 dní"], ["Výpovědní lhůta", "30 dní"], ["Platforma", "Trustera — trustera.cz"]].map(([k, v]) => (
  ...
  return (
    <div style={{ maxWidth: 680, margin: "40px auto", padding: "0 24px" }}>
      <button onClick={() => setPage("new-listing")} style={{ background: "none", border: "none", cursor: "pointer", color: "#888", fontSize: 14, marginBottom: 24, fontFamily: "'Inter', sans-serif" }}>← Zpět</button>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
        <div style={{ width: 44, height: 44, background: "#0F0F0F", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ color: "#1B4FD8", fontSize: 20 }}>📋</span></div>
        <div>
      ...
      <Card style={{ padding: 28, marginBottom: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
          {[
            ...
            { icon: "⚖️", title: "Právní podpora", desc: "Trustera Legal zajistí přípravu smluvní dokumentace (rezervační a kupní smlouva)." },
            { icon: "📣", title: "Distribuce", desc: "Inzerát bude zveřejněn na platformě Trustera a partnerských portálech." },
          ].map(item => (
            <div key={item.title} style={{ background: "#F0F4FF", borderRadius: 12, padding: 16 }}>
        ...
        <div style={{ background: "#EFF6FF", border: "1px solid #BFDBFE", borderRadius: 10, padding: "12px 16px", marginBottom: 20, fontSize: 12, color: "#1E3A8A", lineHeight: 1.6 }}>
          ⚠️ Toto je zjednodušené shrnutí smlouvy pro demo účely. Plné znění smlouvy obdržíte na email.
        </div>

        <div onClick={() => setAgreed(v => !v)} style={{ padding: "16px 18px", background: agreed ? "#f0fdf4" : "#F0F4FF", borderRadius: 12, border: "1.5px solid " + (agreed ? "#6ee7b7" : "#E2E8F4"), cursor: "pointer", marginBottom: 20 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
...
const Nav = ({ setPage, user, setUser }) => {
  const dashboard = user?.role === "SELLER" ? "seller-dash" : user?.role === "ADMIN" ? "admin-dash" : user?.role === "AGENT" ? "agent-dash" : "buyer-dash";
  return (
    <nav aria-label="Hlavní navigace" style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(245,246,242,.92)",
      backdropFilter: "blur(18px)", borderBottom: "1px solid rgba(19,35,29,.09)", padding: "0 28px",
      display: "flex", alignItems: "center", justifyContent: "space-between", height: 68 }}>
      <button onClick={() => setPage("home")} aria-label="Trustera – domovská stránka"
        style={{ display: "flex", alignItems: "center", gap: 11, cursor: "pointer", border: 0, background: "transparent", color: "var(--ink)" }}>
        <span aria-hidden="true" style={{ width: 36, height: 36, background: "var(--brand)", borderRadius: 11, display: "grid", placeItems: "center",
          color: "#fff", fontWeight: 800, fontFamily: "'DM Serif Display', serif", boxShadow: "0 8px 18px rgba(22,92,70,.2)" }}>V</span>
        <span style={{ fontWeight: 750, fontSize: 16, letterSpacing: ".12em" }}>Trustera</span>
      </button>
      <div style={{ display: "flex", alignItems: "center", gap: 30, fontSize: 14 }}>
        <button onClick={() => setPage("listings")} style={{ cursor: "pointer", fontWeight: 600, color: "var(--muted)", border: 0, background: "transparent" }}>Nemovitosti</button>
        {user && <button onClick={() => setPage(dashboard)} style={{ cursor: "pointer", fontWeight: 600, color: "var(--muted)", border: 0, background: "transparent" }}>Můj přehled</button>}
      </div>
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        {user ? (
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 13, fontWeight: 650 }}>{user.nickname}</div>
              <div style={{ fontSize: 11, color: "var(--muted)" }}>{user.role === "BUYER" ? "Kupující" : user.role === "SELLER" ? "Prodávající" : user.role === "AGENT" ? "Agent" : "Administrátor"}</div>
            </div>
            <button aria-label="Odhlásit se" title="Odhlásit se" onClick={() => { setUser(null); setPage("home"); }}
              style={{ width: 38, height: 38, background: "var(--ink)", border: 0, borderRadius: 12, cursor: "pointer", color: "#fff", fontWeight: 700 }}>
              {user.nickname?.[0]?.toUpperCase() || "U"}
            </button>
          </div>
        ) : <><Btn variant="ghost" small onClick={() => setPage("login")}>Přihlásit</Btn><Btn small onClick={() => setPage("register")}>Vytvořit účet</Btn></>}
      </div>
    </nav>
  );
};
...
const LoginPage = ({ setPage, setUser }) => {
  ...
  return (
    <div style={{ minHeight: "calc(100vh - 60px)", display: "grid", gridTemplateColumns: "1fr 1fr" }}>
      <div style={{ background: "#0F0F0F", display: "flex", flexDirection: "column", justifyContent: "space-between", padding: 48 }}>
        <blockquote><p style={{ ...D, color: "#fff", fontSize: 24, fontWeight: 600, lineHeight: 1.5, marginBottom: 18 }}>"Trustera mi ušetřilo přes 200 000 Kč na provizi."</p><footer style={{ color: "#888", fontSize: 13 }}>Tomáš Procházka · Prodal v Praze 2</footer></blockquote>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {DEMO_ACCOUNTS.map(acc => (
              <button key={acc.nickname} onClick={() => loginWith(acc)} disabled={loading}
                style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", borderRadius: 12, border: "1px solid #333", background: "#252525", cursor: "pointer", fontFamily: "'Inter', sans-serif", transition: "all 0.15s" }}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 18, textAlign: "center" }}>{[["50%", "Nižší provize"], ["94%", "Přesnost AI"], ["2,1 mld", "Objem transakcí"]].map(([v, lb]) => (<div key={lb}><div style={{ ...D, color: "#1B4FD8", fontSize: 26, fontWeight: 800 }}>{v}</div><div style={{ color: "#666", fontSize: 12, marginTop: 3 }}>{lb}</div></div>))}</div>
      </div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 48 }}>
        <div style={{ width: "100%", maxWidth: 380 }}>
          <h1 style={{ ...D, fontSize: 30, fontWeight: 800, marginBottom: 5 }}>Přihlásit se</h1>
          <p style={{ color: "#888", marginBottom: 24, fontSize: 14 }}>Nemáte účet? <span onClick={() => setPage("register")} style={{ color: "#0F0F0F", fontWeight: 700, cursor: "pointer", textDecoration: "underline" }}>Zaregistrujte se</span></p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 22 }}>
            {[{ r: "BUYER", label: "🏠 Kupující" }, { r: "SELLER", label: "🏗 Prodávající" }].map(({ r, label }) => (<button key={r} onClick={() => setRole(r)} style={{ padding: "10px 6px", borderRadius: 10, border: "1.5px solid " + (role === r ? "#0F0F0F" : "#E2E8F4"), background: role === r ? "#0F0F0F" : "#fff", color: role === r ? "#fff" : "#888", fontWeight: 700, fontSize: 12, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>{label}</button>))}
          </div>
          <Card style={{ padding: 26 }}>
...
const RegisterPage = ({ setPage, setUser, isSeller }) => {
  ...
  if (step === "done") return (
    <div style={{ maxWidth: 480, margin: "80px auto", padding: "0 24px", textAlign: "center" }}>
      ...
      <h1 style={{ ...D, fontSize: 28, fontWeight: 800, marginBottom: 8 }}>Vítejte na Trustera!</h1>
    ...
    <div style={{ maxWidth: 480, margin: "60px auto", padding: "0 24px" }}>
      ...
      <Card style={{ padding: 28 }}>
          ...
          {code.map((digit, i) => (
            <input
              ...
              style={{
                ...
                borderRadius: 12, border: "2px solid " + (digit ? "#0F0F0F" : "#E2E8F4"),
                fontFamily: "'Inter', sans-serif", outline: "none",
                background: digit ? "#F0F4FF" : "#fff", transition: "all 0.15s"
              }}
            />
          ))}
        </div>
        ...
        <div style={{ textAlign: "center", marginTop: 16 }}>
          <button onClick={() => { setError(""); setCode(["", "", "", "", "", ""]); }} style={{ background: "none", border: "none", fontSize: 13, color: "#888", cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>
            Nepřišel kód? Zaslat znovu
          </button>
        </div>
      </Card>

      <button onClick={() => setStep("form")} style={{ display: "block", margin: "16px auto 0", background: "none", border: "none", fontSize: 13, color: "#bbb", cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>
        ← Zpět na registraci
      </button>
    </div>
  );
  ...
  return (
    <div style={{ maxWidth: 480, margin: "52px auto", padding: "0 24px" }}>
      ...
      <p style={{ color: "#888", marginBottom: 28, fontSize: 14 }}>Máte účet? <span onClick={() => setPage("login")} style={{ color: "#0F0F0F", fontWeight: 700, cursor: "pointer", textDecoration: "underline" }}>Přihlaste se</span></p>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 24 }}>
        {[{ value: "BUYER", label: "🏠 Kupující", desc: "Hledám nemovitost" }, { value: "SELLER", label: "🏗 Prodávající", desc: "Chci prodat" }].map(r => (
          <button key={r.value} onClick={() => setRole(r.value)} style={{ padding: "14px 10px", borderRadius: 13, border: "2px solid " + (role === r.value ? "#0F0F0F" : "#E2E8F4"), background: role === r.value ? "#F0F4FF" : "#fff", cursor: "pointer", textAlign: "left", fontFamily: "'Inter', sans-serif" }}>
      ...
      <Card style={{ padding: 26 }}>
        ...
        <div style={{ background: "#F0F4FF", borderRadius: 9, padding: "10px 13px", fontSize: 12, color: "#888", marginBottom: 16, lineHeight: 1.5 }}>
  ...
  return (
    <div>
      <div style={{ padding: "52px 32px 40px", maxWidth: 1100, margin: "0 auto", textAlign: "center" }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "#EFF6FF", border: "1px solid #BFDBFE", color: "#1E3A8A", fontSize: 12, fontWeight: 700, padding: "6px 16px", borderRadius: 99, marginBottom: 28 }}>✦ REALITNÍ PLATFORMA NOVÉ GENERACE</div>
        <h1 style={{ ...D, fontSize: "clamp(38px,5vw,72px)", fontWeight: 800, lineHeight: 1.05, margin: "0 0 24px" }}>Kupujte a prodávejte<br /><span style={{ color: "#1B4FD8" }}>chytřeji.</span></h1>
      ...
      <div style={{ background: "#0F0F0F", padding: "64px 32px" }}>
        <div style={{ maxWidth: 820, margin: "0 auto" }}>
          ...
          <Card style={{ background: "#252525", border: "1px solid #333" }}>
            ...
            {vr && !vl && (<div style={{ padding: "0 24px 24px" }}><div style={{ height: 1, background: "#333", marginBottom: 20 }} /><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 24 }}><div><div style={{ color: "#888", fontSize: 11, fontWeight: 600, marginBottom: 8 }}>ODHAD</div><div style={{ ...D, color: "#1B4FD8", fontSize: 26, fontWeight: 800 }}>{fmt(vr.value)}</div></div><div><div style={{ color: "#888", fontSize: 11, fontWeight: 600, marginBottom: 8 }}>PŘESNOST</div><div style={{ color: "#fff", fontSize: 26, fontWeight: 700 }}>{Math.round(vr.confidence * 100)}%</div></div><div><div style={{ color: "#888", fontSize: 11, fontWeight: 600, marginBottom: 8 }}>TREND</div><div style={{ color: "#34d399", fontSize: 20, fontWeight: 700 }}>↗ Rostoucí</div></div></div></div>)}
          </Card>
        </div>
      </div>
      <div style={{ padding: "48px 32px", maxWidth: 1100, margin: "0 auto" }}>
      {/* #9 — Kalkulačka úspor */}
          <div style={{ background: "#F0F4FF", borderTop: "1px solid #E8EDF8", padding: "56px 32px", marginLeft: -32, marginRight: -32 }}>
            <div style={{ maxWidth: 800, margin: "0 auto" }}>
              <div style={{ textAlign: "center", marginBottom: 36 }}>
                <h2 style={{ ...D, fontSize: 32, fontWeight: 800, marginBottom: 8 }}>Kolik ušetříte s Trustera?</h2>
                <p style={{ color: "#888", fontSize: 15 }}>Posuňte slider a zjistěte rozdíl v provizi</p>
              </div>
              <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #E8EDF8", padding: 32 }}>
                <div style={{ marginBottom: 28 }}>
                  ...
                  <input type="range" min={5} max={50} step={1} value={price} onChange={e => setPrice(+e.target.value)}
                    style={{ width: "100%", accentColor: "#1B4FD8", height: 6 }} />
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
                  <div style={{ background: "#f0fdf4", border: "2px solid #6ee7b7", borderRadius: 16, padding: 24, textAlign: "center" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 12 }}>
                      <div style={{ width: 24, height: 24, background: "#0F0F0F", borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ color: "#1B4FD8", fontWeight: 900, fontSize: 11 }}>V</span></div>
                      <span style={{ fontWeight: 800, fontSize: 14 }}>Trustera</span>
                    </div>
                  ...
                  <div style={{ background: "#F0F4FF", border: "1.5px solid #E2E8F4", borderRadius: 16, padding: 24, textAlign: "center" }}>
                ...
                <div style={{ background: "#0F0F0F", borderRadius: 14, padding: "20px 28px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div style={{ color: "#888", fontSize: 14 }}>Vaše úspora s Trustera</div>
                  <div style={{ ...D, fontSize: 32, fontWeight: 800, color: "#1B4FD8" }}>{fmtFull(savings)}</div>
                </div>
              </div>
            </div>
          </div>
...
const ListingCard = ({ l, setPage }) => (
  <Card onClick={() => setPage("listing-" + l.id)} style={{ padding: 0 }}>
    ...
    <div style={{ padding: 18 }}>
      ...
      <div style={{ fontSize: 11, color: "#1B4FD8", fontWeight: 600 }}>✦ AI: {fmt(l.aiValuation)}</div>
    </div>
  </Card>
);
...
const ListingsPage = ({ setPage }) => {
  ...
  const CheckBtn = ({ active, onClick, children }) => (
    <button onClick={onClick} style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 12px", borderRadius: 10, border: "1.5px solid " + (active ? "#0F0F0F" : "#E2E8F4"), background: active ? "#0F0F0F" : "#fff", color: active ? "#fff" : "#555", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif", width: "100%", marginBottom: 6 }}>
      <div style={{ width: 16, height: 16, borderRadius: 4, border: "2px solid " + (active ? "#fff" : "#ccc"), background: active ? "#fff" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
        {active && <span style={{ color: "#0F0F0F", fontSize: 10, fontWeight: 900 }}>✓</span>}
      </div>
      {children}
    </button>
  );
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <input placeholder="🔍  Hledat…" value={filters.search} onChange={setFE("search")}
            style={{ padding: "10px 16px", borderRadius: 12, border: "1.5px solid #E2E8F4", fontSize: 14, fontFamily: "'Inter', sans-serif", width: 240 }} />
          <button onClick={() => setSidebarOpen(s => !s)} style={{ display: "flex", alignItems: "center", gap: 7, padding: "10px 16px", borderRadius: 12, border: "1.5px solid " + (sidebarOpen ? "#0F0F0F" : "#E2E8F4"), background: sidebarOpen ? "#0F0F0F" : "#fff", color: sidebarOpen ? "#fff" : "#555", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>
            ⚙️ Filtry {activeCount > 0 && <span style={{ background: "#1B4FD8", color: "#fff", borderRadius: 99, fontSize: 11, fontWeight: 700, padding: "1px 7px" }}>{activeCount}</span>}
          </button>
          {activeCount > 0 && <button onClick={resetFilters} style={{ padding: "10px 14px", borderRadius: 12, border: "1.5px solid #fee2e2", background: "#fff", color: "#991b1b", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>✕ Reset</button>}
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: sidebarOpen ? "260px 1fr" : "1fr", gap: 28 }}>
        ...
        {sidebarOpen && (
          <div style={{ alignSelf: "start", position: "sticky", top: 76 }}>
            <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #E8EDF8", padding: 22 }}>
              ...
              <SidebarSection title="CENA (MIL. KČ)">
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  <div>
                    ...
                    <input type="number" value={filters.priceMin} onChange={setFE("priceMin")} placeholder="0" style={{ width: "100%", padding: "9px 10px", borderRadius: 9, border: "1.5px solid #E2E8F4", fontSize: 13, fontFamily: "'Inter', sans-serif", boxSizing: "border-box" }} />
                  </div>
                  <div>
                    <div style={{ fontSize: 11, color: "#bbb", marginBottom: 4 }}>Do</div>
                    <input type="number" value={filters.priceMax} onChange={setFE("priceMax")} placeholder="100" style={{ width: "100%", padding: "9px 10px", borderRadius: 9, border: "1.5px solid #E2E8F4", fontSize: 13, fontFamily: "'Inter', sans-serif", boxSizing: "border-box" }} />
                  </div>
                </div>
                <div style={{ display: "flex", gap: 6, marginTop: 8, flexWrap: "wrap" }}>
                  {[5, 10, 20, 50].map(v => (
                    <button key={v} onClick={() => setFilters(f => ({ ...f, priceMax: v }))} style={{ padding: "4px 10px", borderRadius: 99, border: "1.5px solid #E2E8F4", background: +filters.priceMax === v ? "#0F0F0F" : "#fff", color: +filters.priceMax === v ? "#fff" : "#555", fontSize: 11, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>do {v}M</button>
                  ))}
                </div>
              </SidebarSection>
              ...
              <SidebarSection title="PLOCHA (M²)">
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  <div>
                    <div style={{ fontSize: 11, color: "#bbb", marginBottom: 4 }}>Od</div>
                    <input type="number" value={filters.sizeMin} onChange={setFE("sizeMin")} placeholder="0" style={{ width: "100%", padding: "9px 10px", borderRadius: 9, border: "1.5px solid #E2E8F4", fontSize: 13, fontFamily: "'Inter', sans-serif", boxSizing: "border-box" }} />
                  </div>
                  <div>
                    <div style={{ fontSize: 11, color: "#bbb", marginBottom: 4 }}>Do</div>
                    <input type="number" value={filters.sizeMax} onChange={setFE("sizeMax")} placeholder="∞" style={{ width: "100%", padding: "9px 10px", borderRadius: 9, border: "1.5px solid #E2E8F4", fontSize: 13, fontFamily: "'Inter', sans-serif", boxSizing: "border-box" }} />
                  </div>
                </div>
              </SidebarSection>
              ...
              <SidebarSection title="POČET POKOJŮ (MIN.)">
                <div style={{ display: "flex", gap: 6 }}>
                  {["", "1", "2", "3", "4", "5+"].map(v => (
                    <button key={v} onClick={() => setF("bedroomsMin")(v === "5+" ? "5" : v)} style={{ flex: 1, padding: "8px 4px", borderRadius: 9, border: "1.5px solid " + (filters.bedroomsMin === (v === "5+" ? "5" : v) ? "#0F0F0F" : "#E2E8F4"), background: filters.bedroomsMin === (v === "5+" ? "5" : v) ? "#0F0F0F" : "#fff", color: filters.bedroomsMin === (v === "5+" ? "5" : v) ? "#fff" : "#555", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>{v || "Vše"}</button>
                  ))}
                </div>
              </SidebarSection>

              <SidebarSection title="LOKALITA / ČTVRŤ">
                <select value={filters.district} onChange={setFE("district")} style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: "1.5px solid #E2E8F4", fontSize: 13, fontFamily: "'Inter', sans-serif", background: "#fff" }}>
              ...
              {activeCount > 0 && (
                <button onClick={resetFilters} style={{ width: "100%", padding: "10px", borderRadius: 10, border: "1.5px solid #fee2e2", background: "#fff5f5", color: "#991b1b", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>
...
const ListingDetail = ({ id, setPage, user }) => {
      ...
      <div style={{ background: "#f0fdf4", border: "1px solid #6ee7b7", borderRadius: 16, padding: 24, marginBottom: 28, textAlign: "left" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          {[{ l: "VAŠE NABÍDKA", v: fmtFull(amount) }, { l: "VYVOLÁVACÍ CENA", v: fmtFull(l.askingPrice) }, { l: "FINANCOVÁNÍ", v: financing === "own" ? "Vlastní zdroje" : financing === "mortgage" ? `Hypotéka (${mortgageBank || "banka"})` : "Kombinace" }, { l: "STATUS", v: "⏳ Čeká na odpověď" }].map(item => (
            <div key={item.l} style={{ background: "#fff", borderRadius: 10, padding: "10px 14px" }}>
              <div style={{ fontSize: 10, color: "#999", fontWeight: 600, marginBottom: 3 }}>{item.l}</div>
              <div style={{ fontWeight: 700, fontSize: 13, color: item.l === "STATUS" ? "#d97706" : item.l === "VAŠE NABÍDKA" ? "#059669" : "#0F0F0F" }}>{item.v}</div>
            </div>
          ))}
        </div>
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 24px" }}>
      <button onClick={() => setPage("listings")} style={{ background: "none", border: "none", cursor: "pointer", color: "#888", fontSize: 14, marginBottom: 22, fontFamily: "'Inter', sans-serif" }}>← Zpět</button>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 370px", gap: 40 }}>
        <div>
          ...
          {imgs.length > 1 && (<div style={{ display: "flex", gap: 8, marginBottom: 26 }}>{imgs.map((im, i) => (<div key={i} onClick={() => setActiveImg(i)} style={{ width: 76, height: 57, borderRadius: 9, overflow: "hidden", cursor: "pointer", border: i === activeImg ? "2.5px solid #1B4FD8" : "2.5px solid transparent", opacity: i === activeImg ? 1 : 0.6 }}><img src={im} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /></div>))}</div>)}
          ...
          {/* AI Ocenění */}
          <Card style={{ padding: 22, background: "#0F0F0F", border: "none", marginBottom: 16 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 18 }}>
              <div><div style={{ color: "#777", fontSize: 10, fontWeight: 600, marginBottom: 5 }}>AI ODHAD</div><div style={{ ...D, color: "#1B4FD8", fontSize: 22, fontWeight: 800 }}>{fmt(l.aiValuation)}</div><div style={{ color: diff > 0 ? "#34d399" : "#f87171", fontSize: 11 }}>{diff > 0 ? "▲" : "▼"} {Math.abs(diff).toFixed(1)}%</div></div>
          ...
          <Card style={{ padding: 22, marginBottom: 16 }}>
            ...
            <div style={{ position: "relative", paddingLeft: 20 }}>
              <div style={{ position: "absolute", left: 7, top: 8, bottom: 8, width: 2, background: "#EEF2FB" }} />
              {priceHistory.map((item, i) => (
                <div key={i} style={{ display: "flex", gap: 14, marginBottom: i < priceHistory.length - 1 ? 20 : 0, position: "relative" }}>
                  <div style={{ position: "absolute", left: -13, top: 4, width: 10, height: 10, borderRadius: "50%", background: item.type === "published" ? "#059669" : "#1B4FD8", border: "2px solid #fff", flexShrink: 0 }} />
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ ...D, fontSize: 16, fontWeight: 800, color: item.type === "reduced" ? "#1B4FD8" : "#0F0F0F" }}>{fmtFull(item.price)}</div>
                  </div>
                  {item.type === "reduced" && (
                    <div style={{ background: "#EFF6FF", border: "1px solid #BFDBFE", borderRadius: 8, padding: "4px 10px", fontSize: 11, color: "#1E3A8A", fontWeight: 600, alignSelf: "center" }}>
          ...
          <Card style={{ padding: 22 }}>
            ...
            <div style={{ marginBottom: 14 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#888", marginBottom: 6 }}>
                ...
                <span style={{ fontWeight: 700, color: "#0F0F0F" }}>{ltv}% = {fmtFull(Math.round(loanAmount))}</span>
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
              ...
              <div style={{ background: "#0F0F0F", borderRadius: 12, padding: 14 }}>
                <div style={{ fontSize: 10, color: "#888", fontWeight: 600, marginBottom: 4 }}>MĚSÍČNÍ SPLÁTKA</div>
                <div style={{ ...D, fontWeight: 800, fontSize: 17, color: "#1B4FD8" }}>{fmtFull(monthly)}</div>
              </div>
            </div>
          </Card>
        </div>
        ...
        <div style={{ position: "sticky", top: 76, alignSelf: "start" }}>
          <Card style={{ padding: 26 }}>
            ...
            <div style={{ fontSize: 12, color: "#1B4FD8", fontWeight: 600, marginBottom: 18 }}>✦ AI odhad: {fmt(l.aiValuation)}</div>
            <div style={{ background: "#EFF6FF", border: "1px solid #BFDBFE", borderRadius: 10, padding: "10px 14px", marginBottom: 16, fontSize: 12, color: "#1E3A8A" }}>🔒 Rezervační poplatek: <strong>50 000 Kč</strong></div>
            {!showOffer ? (
              ...
              <div>
                {ail ? <div style={{ textAlign: "center", padding: "12px", color: "#888", fontSize: 13 }}>AI analyzuje…</div> : ai && (
                  <div style={{ background: "#0F0F0F", borderRadius: 12, padding: 14, marginBottom: 14 }}>
                    <div style={{ color: "#1B4FD8", fontSize: 17, fontWeight: 800, marginBottom: 3 }}>{fmtFull(ai.suggested)}</div>
                ...
                {/* #8 — Competing offers */}
                <div style={{ marginBottom: 14, padding: "14px 16px", background: "#F0F4FF", borderRadius: 11, border: "1px solid #E2E8F4" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                    <span style={{ fontWeight: 700, color: "#0F0F0F", fontSize: 13 }}>{competingOffers.length} další nabídky</span>
                    <span style={{ fontSize: 12, color: "#888" }}>na tuto nemovitost</span>
                  </div>
                  {/* Barevná škála: šedá = nízká, zelená = střední, zlatá = blízko ceny */}
                  <div style={{ position: "relative", height: 10, background: "#E2E8F4", borderRadius: 99, marginBottom: 8 }}>
                    <div style={{ position: "absolute", left: 0, top: 0, height: "100%", width: "100%", borderRadius: 99, background: "linear-gradient(to right, #e0dbd4 0%, #6ee7b7 40%, #1B4FD8 80%, #f87171 100%)" }} />
                    {/* Ostatní nabídky */}
                    ...
                    {(() => {
                      ...
                      return <div style={{ position: "absolute", top: -4, left: yourPos + "%", width: 18, height: 18, borderRadius: "50%", background: "#0F0F0F", border: "3px solid #1B4FD8", transform: "translateX(-50%)", boxShadow: "0 1px 4px rgba(0,0,0,0.3)" }} title="Vaše nabídka" />;
                    })()}
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 12, marginTop: 10, fontSize: 11 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 5 }}><div style={{ width: 10, height: 10, borderRadius: "50%", background: "#0F0F0F", border: "2px solid #1B4FD8" }} /><span style={{ color: "#888" }}>Vaše nabídka</span></div>
                  </div>
                </div>
                ...
                <div style={{ marginBottom: 12 }}>
                  ...
                  <input type="number" value={amount} onChange={e => setAmount(+e.target.value)} style={{ width: "100%", padding: "11px 13px", borderRadius: 11, border: "1.5px solid #E2E8F4", fontSize: 16, fontWeight: 700, boxSizing: "border-box" }} />
                  {amount > 0 && (
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6 }}>
                    {[{ id: "own", label: "💰", sub: "Vlastní" }, { id: "mortgage", label: "🏦", sub: "Hypotéka" }, { id: "combined", label: "🔀", sub: "Kombinace" }].map(f => (
                      <button key={f.id} onClick={() => setOfferFinancing(f.id)} style={{ padding: "10px 6px", borderRadius: 10, border: "1.5px solid " + (financing === f.id ? "#0F0F0F" : "#E2E8F4"), background: financing === f.id ? "#0F0F0F" : "#fff", color: financing === f.id ? "#fff" : "#555", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "'Inter', sans-serif", textAlign: "center", lineHeight: 1.4 }}>
                  ...
                  {financing === "mortgage" && (
                    <input value={mortgageBank} onChange={e => setOfferBank(e.target.value)} placeholder="Název banky (např. Česká spořitelna)…" style={{ marginTop: 8, width: "100%", padding: "9px 12px", borderRadius: 9, border: "1.5px solid #E2E8F4", fontSize: 12, fontFamily: "'Inter', sans-serif", boxSizing: "border-box" }} />
                  )}
                  {financing === "combined" && (
                    <div style={{ marginTop: 8, background: "#F0F4FF", borderRadius: 9, padding: "10px 12px", fontSize: 11, color: "#888" }}>
...
const SellerDash = ({ setPage, user }) => {
  ...
  return (
    <div style={{ maxWidth: 1050, margin: "0 auto", padding: "28px 24px" }}>
      ...
      {isDemo && (
        <div onClick={() => setTab("offers")} style={{ background: "linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 100%)", borderRadius: 16, padding: "18px 24px", marginBottom: 22, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", boxShadow: "0 4px 20px rgba(0,0,0,0.12)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <div style={{ width: 44, height: 44, borderRadius: 12, background: "#1B4FD8", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0 }}>🔔</div>
            <div>
              <div style={{ color: "#fff", fontWeight: 800, fontSize: 15, marginBottom: 3 }}>Vaše nemovitost byla rezervována!</div>
              <div style={{ color: "#1B4FD8", fontSize: 13 }}>Kupující složil rezervační poplatek 50 000 Kč — identita smluvních stran byla odhalena.</div>
            </div>
          </div>
          <div style={{ color: "#1B4FD8", fontSize: 20, flexShrink: 0 }}>→</div>
        </div>
      )}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
      ...
      {isDemo && (
        <div style={{ background: "#fff", border: "2px solid #1B4FD8", borderRadius: 16, padding: 24, marginBottom: 20 }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: 16, marginBottom: 18 }}>
            <div style={{ width: 48, height: 48, borderRadius: 14, background: "#0F0F0F", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, flexShrink: 0 }}>🎉</div>
            <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
            ...
            <div style={{ background: "#F0F4FF", borderRadius: 12, padding: 16 }}>
              ...
              <div style={{ fontSize: 12, color: "#1B4FD8", fontWeight: 600, marginTop: 4 }}>✦ Kupní cena: 19 500 000 Kč</div>
            </div>
          </div>
      ...
      {pendingCount > 0 && (
        <div style={{ background: "#EFF6FF", border: "1px solid #BFDBFE", borderRadius: 14, padding: "14px 20px", marginBottom: 20, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ fontSize: 13, color: "#1E3A8A", fontWeight: 600 }}>📅 Máte <strong>{pendingCount}</strong> nové žádosti o prohlídku čekající na potvrzení</div>
          <button onClick={() => setTab("viewings")} style={{ background: "#1E3A8A", color: "#fff", border: "none", borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>Zobrazit →</button>
        </div>
      )}
      <div style={{ display: "flex", gap: 4, marginBottom: 22, borderBottom: "1px solid #E2E8F4" }}>
        {[{ id: "listings", label: "Moje inzeráty" }, { id: "offers", label: "Příchozí nabídky" + (isDemo ? " (2)" : "") }, { id: "viewings", label: "Prohlídky" + (pendingCount > 0 ? " (" + pendingCount + ")" : "") }].map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{ padding: "9px 18px", background: "none", border: "none", borderBottom: tab === t.id ? "2px solid #0F0F0F" : "2px solid transparent", color: tab === t.id ? "#0F0F0F" : "#888", fontWeight: tab === t.id ? 700 : 500, fontSize: 13, cursor: "pointer", fontFamily: "'Inter', sans-serif", marginBottom: -1 }}>{t.label}</button>
        ))}
      </div>
      ...
      {tab === "listings" && (allListings.length === 0 ? (
        ...
        <Card key={l.id} style={{ padding: 0, overflow: "hidden", marginBottom: 16 }}>
          <div style={{ display: "flex" }}>
            ...
            <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 8, justifyContent: "center", borderLeft: "1px solid #EEF2FB" }}>
              ...
              <Card key={v.id} style={{ padding: 22 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div>
                    ...
                    <div style={{ display: "flex", gap: 10 }}>
                      <span style={{ background: "#F0F4FF", padding: "5px 12px", borderRadius: 8, fontSize: 12, fontWeight: 600 }}>📅 {isMock ? v.date : v.slot?.date}</span>
                      <span style={{ background: "#F0F4FF", padding: "5px 12px", borderRadius: 8, fontSize: 12, fontWeight: 600 }}>🕐 {isMock ? v.time : v.slot?.time}</span>
                    </div>
                  </div>
    ...
    <div style={{ maxWidth: 900, margin: "32px auto", padding: "0 24px" }}>
        ...
        <div>
          ...
          <Card style={{ padding: 22 }}>
            ...
            {[
              ...
              <div key={i} style={{ display: "flex", gap: 12, padding: "10px 0", borderBottom: i < 3 ? "1px solid #F0F4FF" : "none", fontSize: 13 }}>
      ...
      {tab === "distribution" && (
        <Card style={{ padding: 26 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {PORTALS.map(p => (
              <div key={p.name} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 18px", background: "#F0F4FF", borderRadius: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div style={{ width: 10, height: 10, borderRadius: "50%", background: p.status === "active" ? "#059669" : p.status === "processing" ? "#d97706" : "#E2E8F4", flexShrink: 0 }} />
              </div>
            ))}
          </div>
        ...
        <Card style={{ padding: 26 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 24 }}>
            {[
              ...
              { name: "Zkopírovat odkaz", icon: copied ? "✓" : "🔗", color: copied ? "#059669" : "#0F0F0F", bg: copied ? "#d1fae5" : "#F0F4FF" },
            ].map(s => (
              <button key={s.name} onClick={s.name === "Zkopírovat odkaz" ? copyLink : () => {}}
                style={{ padding: "16px 8px", borderRadius: 14, border: "none", background: s.bg, cursor: "pointer", textAlign: "center", fontFamily: "'Inter', sans-serif", transition: "all 0.15s" }}>
                <div style={{ fontSize: 28, marginBottom: 6 }}>{s.icon}</div>
                <div style={{ fontSize: 12, fontWeight: 700, color: s.color }}>{s.name === "Zkopírovat odkaz" && copied ? "Zkopírováno!" : s.name}</div>
              </button>
            ))}
          </div>
          <div style={{ background: "#F0F4FF", borderRadius: 12, padding: "12px 16px", display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <button onClick={copyLink} style={{ background: "#0F0F0F", color: "#fff", border: "none", borderRadius: 8, padding: "6px 14px", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif", whiteSpace: "nowrap" }}>
      ...
      {tab === "progress" && (
        <div>
          ...
          <Card style={{ padding: 22 }}>
            ...
            <div style={{ display: "flex", gap: 12, alignItems: "center", padding: "12px 16px", background: "#EFF6FF", borderRadius: 12, marginBottom: 12 }}>
              <span style={{ fontSize: 20 }}>📅</span>
              <div>
                <div style={{ fontWeight: 700, fontSize: 13 }}>Probíhají prohlídky</div>
                <div style={{ fontSize: 12, color: "#1E3A8A" }}>3 prohlídky dokončeny · 2 čekají na potvrzení</div>
              </div>
            </div>
...
const LegalWorkflowPage = ({ id, setPage, user }) => {
  ...
  const DocHeader = (title, no) => `<h1>${title}</h1><div class="meta">č. ${no} · Trustera · ${today}</div><div class="warn">⚠️ Vzorový dokument generovaný automaticky. Před podpisem doporučujeme konzultaci s advokátem.</div>`;
  ...
  const printReservation = () => printDoc("Rezervační smlouva", `
    ...
    <div class="clause"><strong>1.</strong> Smlouva nabývá účinnosti uhrazením rezervačního poplatku připsáním na účet platformy Trustera.</div>
  ...
  const printFutureContract = () => printDoc("Smlouva o smlouvě budoucí kupní", `
    ...
    <div class="clause">Smluvní strany se dohodly, že kupní cena bude uhrazena prostřednictvím advokátní úschovy vedenou Trustera Legal, s.r.o. Podmínky uvolnění budou upraveny smlouvou o advokátní úschově, kterou smluvní strany uzavřou nejpozději společně s kupní smlouvou.</div>
  ...
  const printEscrowContract = () => printDoc("Smlouva o advokátní úschově", `
    ...
    <div class="box"><div class="row"><span class="lbl">Advokát (schovatel):</span><span class="val">Trustera Legal, s.r.o.</span></div><div class="row"><span class="lbl">Adresa:</span><span class="val">Národní 12, Praha 1, 110 00</span></div><div class="row"><span class="lbl">IČO:</span><span class="val">12345678</span></div><div class="row"><span class="lbl">Číslo ČAK:</span><span class="val">12345</span></div></div>
    ...
    <div class="sig" style="margin-top:24px;max-width:300px"><strong>Trustera Legal, s.r.o.</strong><br/>Advokát / schovatel<br/>Podpis a razítko: ___________________</div>
    <div style="margin-top:40px;text-align:center;font-size:11px;color:#aaa">Vygenerováno platformou Trustera · trustera.cz · ${today}</div>
  `);
  ...
  const ApprovalBox = ({ label, approved, onToggle, color = "#059669" }) => (
    <div onClick={onToggle} style={{ padding: 16, background: approved ? "#f0fdf4" : "#F0F4FF", borderRadius: 12, border: "1.5px solid " + (approved ? "#6ee7b7" : "#E2E8F4"), cursor: "pointer" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
  ...
  const PdfBtn = ({ onClick, label }) => (
    <button onClick={onClick} style={{ display: "inline-flex", alignItems: "center", gap: 7, background: "#0F0F0F", color: "#fff", border: "none", borderRadius: 9, padding: "9px 18px", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif", marginBottom: 4 }}>
      📄 {label}
    </button>
  );

  const InfoBox = ({ color = "blue", icon, children }) => {
    const colors = { blue: ["#dbeafe", "#1e40af"], amber: ["#EFF6FF", "#1E3A8A"], green: ["#d1fae5", "#065f46"], red: ["#fee2e2", "#991b1b"] };
  ...
  return (
    <div style={{ maxWidth: 980, margin: "32px auto", padding: "0 24px" }}>
      <button onClick={() => setPage("buyer-dash")} style={{ background: "none", border: "none", cursor: "pointer", color: "#888", fontSize: 14, marginBottom: 24, fontFamily: "'Inter', sans-serif" }}>← Zpět na dashboard</button>
      ...
      {/* Stepper */}
      <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #E8EDF8", padding: "18px 20px", marginBottom: 28, overflowX: "auto" }}>
            ...
            return (
              <div key={s.id} style={{ display: "flex", alignItems: "center", flexShrink: 0 }}>
                <div onClick={() => (done || active) && setActiveStep(s.id)} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5, cursor: done || active ? "pointer" : "default", minWidth: 68 }}>
                  <div style={{ width: 34, height: 34, borderRadius: "50%", background: done ? "#059669" : active ? "#0F0F0F" : "#E2E8F4", color: done || active ? "#fff" : "#aaa", display: "flex", alignItems: "center", justifyContent: "center", fontSize: done ? 13 : 14, fontWeight: 700 }}>{done ? "✓" : s.icon}</div>
                  <div style={{ fontSize: 10, fontWeight: active ? 700 : 400, color: active ? "#0F0F0F" : done ? "#059669" : "#aaa", textAlign: "center", lineHeight: 1.3 }}>{s.label}</div>
                </div>
                {i < WORKFLOW_STEPS.length - 1 && <div style={{ width: 24, height: 2, background: done ? "#059669" : "#E2E8F4", margin: "0 2px", marginBottom: 18, flexShrink: 0 }} />}
              </div>
            );
          })}
        </div>
      ...
      {activeStep === "verification" && (
        <StepCard title="🔍 Prověření nemovitosti" subtitle="Před podpisem jakékoli smlouvy je nutné ověřit právní stav nemovitosti v katastru. Toto je klíčový krok — odhalí zástavy, věcná břemena, exekuce nebo spoluvlastníky.">
          <InfoBox color="amber" icon="⚠️">Vždy si vyžádejte aktuální výpis z katastru nemovitostí (LV) přímo z <a href="https://nahlizenidokn.cuzk.cz" target="_blank" rel="noreferrer" style={{ color: "#1E3A8A" }}>nahlizenidokn.cuzk.cz</a>. Výpis musí být ne starší 3 měsíců.</InfoBox>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <textarea value={encumbrances} onChange={e => setEncumbrances(e.target.value)} rows={2} placeholder="Z výpisu z KN opište případná věcná břemena, zástavní práva nebo jiné závady (nebo napište 'Žádné')…" style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: "1.5px solid #E2E8F4", fontSize: 13, fontFamily: "'Inter', sans-serif", resize: "none", boxSizing: "border-box" }} />
          </div>

          <div style={{ marginBottom: 20, padding: "16px 18px", background: coowners ? "#EFF6FF" : "#F0F4FF", borderRadius: 12, border: "1.5px solid " + (coowners ? "#BFDBFE" : "#E2E8F4") }}>
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: coowners ? 12 : 0 }}>
              {[{ v: false, l: "Nemovitost nemá spoluvlastníky" }, { v: true, l: "Má spoluvlastníky" }].map(opt => (
                <button key={String(opt.v)} onClick={() => setCoowners(opt.v)} style={{ padding: "8px 16px", borderRadius: 9, border: "1.5px solid " + (coowners === opt.v ? "#0F0F0F" : "#E2E8F4"), background: coowners === opt.v ? "#0F0F0F" : "#fff", color: coowners === opt.v ? "#fff" : "#555", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>{opt.l}</button>
              ))}
            </div>
            {coowners && (
              <div>
        ...
        <StepCard title="💰 Způsob financování" subtitle="Vyberte způsob úhrady kupní ceny. Ovlivní obsah smluv, lhůty a postup při hypotéce.">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
            {FINANCING_OPTIONS.map(f => (
              <div key={f.id} onClick={() => { setFinancing(f.id); setDeadlineDays(f.deadlineDays); }} style={{ padding: 16, borderRadius: 12, border: "2px solid " + (financing === f.id ? f.color : "#E2E8F4"), background: financing === f.id ? "#f7fffe" : "#fff", cursor: "pointer", transition: "all 0.15s" }}>
                <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 4, color: financing === f.id ? f.color : "#0F0F0F" }}>{f.label}</div>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
              {[30, 45, 60, 90].map(d => (
                <button key={d} onClick={() => { setDeadlineDays(d); setCustomDeadline(""); }} style={{ padding: "8px 16px", borderRadius: 10, border: "1.5px solid " + (deadlineDays === d && !customDeadline ? "#0F0F0F" : "#E2E8F4"), background: deadlineDays === d && !customDeadline ? "#0F0F0F" : "#fff", color: deadlineDays === d && !customDeadline ? "#fff" : "#555", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>{d} dní</button>
              ))}
              <input type="number" value={customDeadline} onChange={e => setCustomDeadline(e.target.value)} placeholder="Vlastní…" style={{ width: 100, padding: "8px 12px", borderRadius: 10, border: "1.5px solid " + (customDeadline ? "#0F0F0F" : "#E2E8F4"), fontSize: 13, fontFamily: "'Inter', sans-serif" }} />
            </div>
        ...
        <StepCard title="📋 Rezervační smlouva" subtitle="Smlouva byla uzavřena a rezervační poplatek uhrazen. Níže je shrnutí podmínek.">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 20 }}>
            {[{ l: "KUPNÍ CENA", v: fmtFull(l.askingPrice) }, { l: "FINANCOVÁNÍ", v: fin?.label || "–" }, { l: "LHŮTA PRO KS", v: addDays(+effectiveDeadline) }].map(item => (
              <div key={item.l} style={{ background: "#F0F4FF", borderRadius: 12, padding: 14 }}><div style={{ fontSize: 10, color: "#999", fontWeight: 600, marginBottom: 5 }}>{item.l}</div><div style={{ fontWeight: 700, fontSize: 13 }}>{item.v}</div></div>
            ))}
          </div>
        ...
        <StepCard title="📝 Smlouva o smlouvě budoucí kupní" subtitle="Obě strany se zaváží uzavřít kupní smlouvu za dohodnutých podmínek. SSBK upravuje i podmínky úschovy a hypotéky.">
          <div style={{ marginBottom: 20 }}>
            {[
              ...
              { l: "Úschova kupní ceny", v: "Trustera Legal, s.r.o." },
              ...
              <div key={row.l} style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderBottom: "1px solid #EEF2FB", fontSize: 13 }}>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            {commentsSent ? (
              ...
              <div>
                ...
                <button onClick={() => setCommentsSent(true)} style={{ background: "none", border: "1.5px solid #E2E8F4", borderRadius: 9, padding: "8px 16px", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>Odeslat připomínky</button>
              </div>
            )}
          </div>
        ...
        <StepCard title="🔐 Smlouva o advokátní úschově" subtitle="Třístranná smlouva mezi kupujícím, prodávajícím a advokátem. Upravuje podmínky uložení a uvolnění kupní ceny. Musí být uzavřena před kupní smlouvou.">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 20 }}>
            <div style={{ background: "#F0F4FF", borderRadius: 12, padding: 16 }}>
              ...
              <div style={{ fontWeight: 700, marginBottom: 4 }}>Trustera Legal, s.r.o.</div>
      ...
      {activeStep === "escrow_payment" && (
        <StepCard title="💳 Úhrada kupní ceny do úschovy" subtitle="Doplatek kupní ceny odešlete na úschovní účet advokáta. Bez přijetí na úschovním účtu nelze podat návrh na vklad.">
          ...
          <div style={{ marginBottom: 20, padding: "16px 18px", background: escrowSent ? "#f0fdf4" : "#F0F4FF", borderRadius: 12, border: "1.5px solid " + (escrowSent ? "#6ee7b7" : "#E2E8F4"), cursor: "pointer" }} onClick={() => setEscrowSent(v => !v)}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
      ...
      {activeStep === "cadastre" && (
        <StepCard title="🏛️ Návrh na vklad do katastru nemovitostí" subtitle="Advokát podá návrh na vklad do 5 pracovních dnů od podpisu kupní smlouvy. Standardní lhůta zápisu je 20–30 dní.">
          ...
          <div style={{ marginTop: 20, paddingTop: 20, borderTop: "1px solid #EEF2FB" }}>
            <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 4 }}>📅 Rezervace termínu u advokáta</div>
            <div style={{ fontSize: 12, color: "#888", marginBottom: 16 }}>Trustera Legal, s.r.o. · Praha 1, Národní 12 · +420 222 333 444</div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 20 }}>
                <div style={{ maxHeight: 260, overflowY: "auto", display: "flex", flexDirection: "column", gap: 6 }}>
                  {lawyerSlots.slice(0, 15).map(slot => (
                    <div key={slot.id} onClick={() => setLawyerSlot(slot)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "11px 14px", borderRadius: 10, border: "1.5px solid " + (lawyerSlot?.id === slot.id ? "#0F0F0F" : "#E2E8F4"), background: lawyerSlot?.id === slot.id ? "#0F0F0F" : "#fff", cursor: "pointer" }}>
                      <span style={{ fontSize: 13, fontWeight: 600, color: lawyerSlot?.id === slot.id ? "#fff" : "#0F0F0F" }}>{slot.date} · {slot.time}</span>
                      {lawyerSlot?.id === slot.id && <span style={{ color: "#1B4FD8", fontWeight: 700 }}>✓</span>}
                    </div>
                  ))}
                </div>
      ...
      {activeStep === "handover" && (
        <StepCard title="🔑 Předání nemovitosti" subtitle="Nemovitost se předává po zápisu vlastnického práva v katastru nemovitostí a uvolnění kupní ceny z úschovy. Sepište předávací protokol.">
          ...
          <div style={{ marginBottom: 20, padding: "16px 18px", background: keysHandedOver ? "#f0fdf4" : "#F0F4FF", borderRadius: 12, border: "1.5px solid " + (keysHandedOver ? "#6ee7b7" : "#E2E8F4"), cursor: "pointer" }} onClick={() => setKeysHandedOver(v => !v)}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  {[
                    ...
                    return (
                      <div key={item.key} onClick={() => setMeterReadings(m => ({ ...m, [item.key]: !m[item.key] }))}
                        style={{ padding: "13px 16px", background: checked ? "#f0fdf4" : "#F0F4FF", borderRadius: 12, border: "1.5px solid " + (checked ? "#6ee7b7" : "#E2E8F4"), cursor: "pointer" }}>
                        <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
...
const PurchaseContractPage = ({ id, setPage, user }) => {
  ...
  const printContract = () => {
    ...
    <h1>KUPNÍ SMLOUVA</h1>
    <div style="text-align:center;color:#666;font-size:13px;margin-bottom:32px">č. ${contractNo} · Trustera · ${today}</div>
    <h2>I. Smluvní strany</h2>
    <div class="highlight">
    ...
    <div style="margin-top:40px;text-align:center;font-size:11px;color:#999">Vygenerováno platformou Trustera · trustera.cz · ${today}</div>
    `);
    win.document.close();
    setTimeout(() => win.print(), 500);
  };
  ...
  return (
    <div style={{ maxWidth: 900, margin: "40px auto", padding: "0 24px" }}>
      <button onClick={() => setPage("buyer-dash")} style={{ background: "none", border: "none", cursor: "pointer", color: "#888", fontSize: 14, marginBottom: 26, fontFamily: "'Inter', sans-serif" }}>← Zpět na dashboard</button>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 28 }}>
        {[
          ...
          <div key={i} style={{ background: s.done ? "#f0fdf4" : "#F0F4FF", border: "1px solid " + (s.done ? "#6ee7b7" : "#E2E8F4"), borderRadius: 12, padding: "14px 16px", display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 24, borderBottom: "1px solid #E2E8F4" }}>
        {[{ id: "contract", label: "📄 Smlouva" }, { id: "comments", label: "💬 Připomínky" }, { id: "lawyer", label: "📅 Advokát" }].map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{ padding: "10px 20px", background: "none", border: "none", borderBottom: tab === t.id ? "2px solid #0F0F0F" : "2px solid transparent", color: tab === t.id ? "#0F0F0F" : "#888", fontWeight: tab === t.id ? 700 : 500, fontSize: 13, cursor: "pointer", fontFamily: "'Inter', sans-serif", marginBottom: -1 }}>{t.label}</button>
        ))}
      </div>
      ...
      {tab === "contract" && (
        <div>
          <Card style={{ padding: 28, marginBottom: 20 }}>
            {/* Hlavička smlouvy */}
            <div style={{ textAlign: "center", paddingBottom: 20, marginBottom: 20, borderBottom: "2px solid #0F0F0F" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 8 }}>
                <div style={{ width: 28, height: 28, background: "#0F0F0F", borderRadius: 7, display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ color: "#1B4FD8", fontWeight: 900, fontSize: 13 }}>V</span></div>
                <span style={{ fontWeight: 800, fontSize: 15, fontFamily: "'Playfair Display', serif" }}>Trustera</span>
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 20 }}>
              ...
              <div style={{ border: "1.5px solid #1B4FD8", borderRadius: 12, padding: 14, background: "#EEF2FF" }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: "#1E3A8A", marginBottom: 5 }}>DOPLATEK</div>
                <div style={{ ...D, fontSize: 18, fontWeight: 800, color: "#1B4FD8" }}>{fmtFull(l.askingPrice - l.reservationFee)}</div>
              </div>
            </div>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              {[
                ...
                <div key={i} style={{ display: "flex", gap: 10, marginBottom: 8, fontSize: 13, color: "#444", lineHeight: 1.5 }}>
                  <span style={{ color: "#1B4FD8", fontWeight: 700, flexShrink: 0 }}>{i + 1}.</span>
                  <span>{c}</span>
                </div>
              ))}
            </div>
            ...
            {/* PDF tlačítko */}
            <div style={{ borderTop: "1px solid #EEF2FB", paddingTop: 18, display: "flex", gap: 10 }}>
              <button onClick={printContract} style={{ display: "flex", alignItems: "center", gap: 8, background: "#0F0F0F", color: "#fff", border: "none", borderRadius: 10, padding: "10px 20px", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>
          ...
          <Card style={{ padding: 24 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              <div onClick={() => setBuyerApproved(v => !v)} style={{ padding: "16px", background: buyerApproved ? "#f0fdf4" : "#F0F4FF", borderRadius: 12, border: "1.5px solid " + (buyerApproved ? "#6ee7b7" : "#E2E8F4"), cursor: "pointer" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div onClick={() => setSellerApproved(v => !v)} style={{ padding: "16px", background: sellerApproved ? "#f0fdf4" : "#F0F4FF", borderRadius: 12, border: "1.5px solid " + (sellerApproved ? "#6ee7b7" : "#E2E8F4"), cursor: "pointer" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            {bothApproved && (
              <div style={{ marginTop: 16, background: "#f0fdf4", border: "1px solid #6ee7b7", borderRadius: 12, padding: "14px 18px", fontSize: 13, color: "#059669", fontWeight: 600 }}>
                ...
                <button onClick={() => setTab("lawyer")} style={{ marginLeft: 12, background: "#059669", color: "#fff", border: "none", borderRadius: 8, padding: "6px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>Rezervovat termín →</button>
              </div>
            )}
          </Card>
        </div>
      ...
      {tab === "comments" && (
        <div>
          <Card style={{ padding: 24, marginBottom: 16 }}>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              {buyerCommentSent && buyerComment ? (
                ...
                <div>
                  <textarea value={buyerComment} onChange={e => setBuyerComment(e.target.value)} rows={3} placeholder="Napište připomínku nebo požadavek na úpravu smlouvy…" style={{ width: "100%", padding: "11px 13px", borderRadius: 11, border: "1.5px solid #E2E8F4", fontSize: 13, fontFamily: "'Inter', sans-serif", resize: "vertical", boxSizing: "border-box", marginBottom: 8 }} />
            ...
            <div style={{ paddingTop: 20, borderTop: "1px solid #EEF2FB" }}>
              ...
              {sellerCommentSent && sellerComment ? (
                ...
                <div>
                  <textarea value={sellerComment} onChange={e => setSellerComment(e.target.value)} rows={3} placeholder="Reagujte na připomínky nebo přidejte vlastní…" style={{ width: "100%", padding: "11px 13px", borderRadius: 11, border: "1.5px solid #E2E8F4", fontSize: 13, fontFamily: "'Inter', sans-serif", resize: "vertical", boxSizing: "border-box", marginBottom: 8 }} />
          ...
          {buyerCommentSent && sellerCommentSent && (
            <div style={{ background: "#EFF6FF", border: "1px solid #BFDBFE", borderRadius: 12, padding: "14px 18px", fontSize: 13, color: "#1E3A8A" }}>
              💡 Obě strany zaslaly připomínky. Doporučujeme konzultaci s advokátem před finálním podpisem.
              <button onClick={() => setTab("lawyer")} style={{ marginLeft: 12, background: "#1E3A8A", color: "#fff", border: "none", borderRadius: 8, padding: "6px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>Rezervovat advokáta →</button>
            </div>
          )}
        </div>
      )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 24 }}>
          <div>
            ...
            <Card style={{ padding: 24, marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 12, color: "#888", marginBottom: 18 }}>Advokátní kancelář Trustera Legal · Praha 1, Národní 12</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 8, maxHeight: 340, overflowY: "auto" }}>
                {slots.slice(0, 20).map(slot => (
                  <div key={slot.id} onClick={() => setSelectedSlot(slot)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderRadius: 10, border: "1.5px solid " + (selectedSlot?.id === slot.id ? "#0F0F0F" : "#E2E8F4"), background: selectedSlot?.id === slot.id ? "#0F0F0F" : "#fff", cursor: "pointer", transition: "all 0.15s" }}>
                    <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
                      <span style={{ fontSize: 20 }}>📅</span>
                      <div>
                        <div style={{ fontWeight: 600, fontSize: 13, color: selectedSlot?.id === slot.id ? "#fff" : "#0F0F0F" }}>{slot.date}</div>
                        <div style={{ fontSize: 12, color: selectedSlot?.id === slot.id ? "#aaa" : "#888" }}>{slot.time}</div>
                      </div>
                    </div>
                    {selectedSlot?.id === slot.id && <span style={{ color: "#1B4FD8", fontWeight: 700 }}>✓ Vybráno</span>}
                  </div>
                ))}
              </div>
            </Card>
              ...
              {lawyerSent ? (
                ...
                <div>
                  ...
                  <div style={{ marginBottom: 16 }}>
                    ...
                    <textarea value={lawyerForm.message} onChange={e => setLawyerForm(f => ({ ...f, message: e.target.value }))} rows={3} placeholder="Případné poznámky nebo dotazy…" style={{ width: "100%", padding: "11px 13px", borderRadius: 11, border: "1.5px solid #E2E8F4", fontSize: 13, fontFamily: "'Inter', sans-serif", resize: "none", boxSizing: "border-box" }} />
                  </div>
...
const BuyerViewings = ({ user, setPage }) => {
  ...
  if (myViewings.length === 0) return (
    <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #E8EDF8", padding: "48px 32px", textAlign: "center" }}>
            ...
            <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "flex", gap: 12, fontSize: 13 }}>
                  <span style={{ background: "#F0F4FF", padding: "5px 12px", borderRadius: 8, fontWeight: 600 }}>📅 {v.slot.date}</span>
                  <span style={{ background: "#F0F4FF", padding: "5px 12px", borderRadius: 8, fontWeight: 600 }}>🕐 {v.slot.time}</span>
                </div>
              </div>
            </div>
            {v.form?.note && <div style={{ marginTop: 12, background: "#F0F4FF", borderRadius: 9, padding: "9px 14px", fontSize: 12, color: "#666" }}>💬 {v.form.note}</div>}
...
const BuyerDash = ({ setPage, user }) => {
  ...
  return (
    <div style={{ maxWidth: 1050, margin: "0 auto", padding: "28px 24px" }}>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 20, borderBottom: "1px solid #E2E8F4" }}>
      ...
      {tab === "offers" && (<div style={{ background: "#fff", borderRadius: 20, border: "1px solid #E8EDF8", padding: "48px 32px", textAlign: "center" }}>
        <div style={{ fontSize: 48, marginBottom: 16 }}>💌</div>
...
const AiDescBtn = ({ form, onGenerated }) => {
  ...
  return (
    <button onClick={generate} disabled={loading} style={{ display: "flex", alignItems: "center", gap: 7, background: loading ? "#F0F4FF" : "#0F0F0F", color: loading ? "#888" : "#1B4FD8", border: "none", borderRadius: 9, padding: "8px 14px", fontSize: 12, fontWeight: 700, cursor: loading ? "default" : "pointer", fontFamily: "'Inter', sans-serif" }}>
      {loading ? (
        <><div style={{ width: 14, height: 14, border: "2px solid #1B4FD8", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} /> AI píše popis…</>
      ) : (
        <>✦ AI popis</>
      )}
    </button>
...
const NewListingPage = ({ setPage, user }) => {
  ...
  return (
    <div style={{ maxWidth: 760, margin: "40px auto", padding: "0 24px" }}>
      <button onClick={() => setPage("seller-dash")} style={{ background: "none", border: "none", cursor: "pointer", color: "#888", fontSize: 14, marginBottom: 26, fontFamily: "'Inter', sans-serif" }}>← Zpět</button>
      ...
      <Card style={{ padding: 24, marginBottom: 20 }}>
        ...
        <div onDrop={handleDrop} onDragOver={e => e.preventDefault()}
          ...
          style={{ border: "2px dashed " + (uploading ? "#1B4FD8" : "#E2E8F4"), borderRadius: 14, padding: "32px 20px", textAlign: "center", cursor: uploading ? "default" : "pointer", background: "#FAFAF9", marginBottom: 16, transition: "all 0.2s" }}>
          <input ref={fileInputRef} type="file" accept="image/*" multiple style={{ display: "none" }} onChange={e => handleFiles(e.target.files)} />
          {uploading ? (
            <div>
              <div style={{ width: 48, height: 48, border: "4px solid #EEF2FB", borderTopColor: "#1B4FD8", borderRadius: "50%", animation: "spin 0.8s linear infinite", margin: "0 auto 14px" }} />
              <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 6 }}>Nahrávám fotky… {uploadProgress}%</div>
              <div style={{ height: 6, background: "#EEF2FB", borderRadius: 99, overflow: "hidden", maxWidth: 200, margin: "0 auto" }}>
                <div style={{ height: "100%", background: "#1B4FD8", borderRadius: 99, width: uploadProgress + "%", transition: "width 0.3s" }} />
              </div>
            </div>
          ) : (
            <div>
        ...
        {photos.length > 0 && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(120px, 1fr))", gap: 10 }}>
            {photos.map((p, i) => (
              <div key={i} style={{ position: "relative", borderRadius: 10, overflow: "hidden", border: i === 0 ? "2.5px solid #1B4FD8" : "2.5px solid transparent" }}>
                <img src={p.preview || p.url} alt="" style={{ width: "100%", height: 90, objectFit: "cover", display: "block" }} />
                {i === 0 && <div style={{ position: "absolute", top: 4, left: 4, background: "#1B4FD8", color: "#fff", fontSize: 10, fontWeight: 700, padding: "2px 7px", borderRadius: 99 }}>TITULNÍ</div>}
                {aiEnhance && <div style={{ position: "absolute", top: 4, right: 30, background: "#0F0F0F", color: "#1B4FD8", fontSize: 9, fontWeight: 700, padding: "2px 6px", borderRadius: 99 }}>✦ AI</div>}
            ...
            {photos.length < 10 && (
              <div onClick={() => fileInputRef.current?.click()} style={{ height: 90, border: "2px dashed #E2E8F4", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#bbb", fontSize: 28 }}>+</div>
            )}
          </div>
        )}
        ...
        {photos.length > 0 && (
          <div style={{ marginTop: 16, padding: "14px 16px", background: aiEnhance ? "#0F0F0F" : "#F0F4FF", borderRadius: 12, border: "1.5px solid " + (aiEnhance ? "#1B4FD8" : "#E2E8F4") }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontSize: 20 }}>✦</span>
                <div>
                  <div style={{ fontWeight: 700, fontSize: 13, color: aiEnhance ? "#1B4FD8" : "#0F0F0F" }}>AI vylepšení fotek</div>
                  <div style={{ fontSize: 11, color: aiEnhance ? "#888" : "#999" }}>Automatická úprava jasu, kontrastu a barev pro maximální atraktivitu</div>
                </div>
              </div>
              <button onClick={() => setAiEnhance(v => !v)} style={{ width: 44, height: 24, borderRadius: 99, background: aiEnhance ? "#1B4FD8" : "#E2E8F4", border: "none", cursor: "pointer", position: "relative", transition: "background 0.2s", flexShrink: 0 }}>
            ...
            {aiEnhance && (
              <div style={{ marginTop: 12, display: "flex", gap: 8, flexWrap: "wrap" }}>
                {["Jas +15%", "Kontrast +10%", "Sytost +8%", "Ostrost", "Odstranění šumu"].map(f => (
                  <span key={f} style={{ fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: 99, background: "rgba(201,168,76,0.2)", color: "#1B4FD8" }}>✦ {f}</span>
                ))}
              </div>
            )}
          </div>
        ...
        {/* #16 — Fotograf */}
        <div style={{ marginTop: 12, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", background: "#F0F4FF", borderRadius: 11 }}>
          <div style={{ fontSize: 12, color: "#888" }}>📸 Chcete profesionální fotografie?</div>
          <button onClick={() => setShowPhotographer(true)} style={{ background: "none", border: "1.5px solid #E2E8F4", borderRadius: 8, padding: "6px 14px", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif", color: "#555", whiteSpace: "nowrap" }}>
            Objednat fotografa →
          </button>
        </div>
                ...
                <div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                    <div>
                      ...
                      <div style={{ fontSize: 13, color: "#888" }}>Trustera Photography · Praha a okolí</div>
                    </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 20 }}>
                    {[{ icon: "📸", label: "Fotografie", desc: "20–30 fotek" }, { icon: "🎬", label: "Video tour", desc: "2 min. video" }, { icon: "🏠", label: "3D scan", desc: "Virtuální prohlídka" }].map(p => (
                      <div key={p.label} style={{ background: "#F0F4FF", borderRadius: 12, padding: "14px 10px", textAlign: "center" }}>
                  ...
                  <div style={{ marginBottom: 14 }}>
                    ...
                    <input type="date" value={photoForm.date} onChange={e => setPhotoForm(f => ({ ...f, date: e.target.value }))} style={{ width: "100%", padding: "11px 13px", borderRadius: 11, border: "1.5px solid #E2E8F4", fontSize: 13, fontFamily: "'Inter', sans-serif", boxSizing: "border-box" }} />
                  </div>
                  <div style={{ marginBottom: 14 }}>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      {["8:00", "10:00", "12:00", "14:00", "16:00"].map(t => (
                        <button key={t} onClick={() => setPhotoForm(f => ({ ...f, time: t }))} style={{ flex: 1, padding: "9px 4px", borderRadius: 9, border: "1.5px solid " + (photoForm.time === t ? "#0F0F0F" : "#E2E8F4"), background: photoForm.time === t ? "#0F0F0F" : "#fff", color: photoForm.time === t ? "#fff" : "#555", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>{t}</button>
                      ))}
                    </div>
                  </div>
                  <div style={{ background: "#EFF6FF", borderRadius: 10, padding: "10px 14px", fontSize: 12, color: "#1E3A8A", marginBottom: 16 }}>
      ...
      <Card style={{ padding: 24, marginBottom: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <div style={{ marginBottom: 14 }}>
            ...
            <select value={form.propertyType} onChange={setF("propertyType")} style={{ width: "100%", padding: "11px 13px", borderRadius: 11, border: "1.5px solid #E2E8F4", fontSize: 13, fontFamily: "'Inter', sans-serif", background: "#fff" }}>
      ...
      <Card style={{ padding: 24, marginBottom: 20 }}>
        ...
        {form.askingPrice && form.internalSize && form.district ? (() => {
          ...
          return (
            <div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 20 }}>
                <div style={{ background: "#0F0F0F", borderRadius: 12, padding: 16, textAlign: "center" }}>
                  <div style={{ fontSize: 10, color: "#888", fontWeight: 600, marginBottom: 6 }}>AI ODHAD</div>
                  <div style={{ ...D, color: "#1B4FD8", fontSize: 20, fontWeight: 800 }}>{fmt(aiVal)}</div>
                  <div style={{ fontSize: 10, color: "#666", marginTop: 4 }}>±7% přesnost</div>
                </div>
                <div style={{ background: "#F0F4FF", borderRadius: 12, padding: 16, textAlign: "center" }}>
                ...
                <div style={{ background: asked > high ? "#fee2e2" : asked < low ? "#EFF6FF" : "#f0fdf4", borderRadius: 12, padding: 16, textAlign: "center" }}>
                  <div style={{ fontSize: 10, color: "#999", fontWeight: 600, marginBottom: 6 }}>VAŠE CENA</div>
                  <div style={{ fontSize: 20, fontWeight: 800, color: asked > high ? "#991b1b" : asked < low ? "#1E3A8A" : "#059669" }}>{diff > 0 ? "+" : ""}{diff}%</div>
                  <div style={{ fontSize: 10, color: "#888", marginTop: 4 }}>{asked > high ? "Nad trhem" : asked < low ? "Pod trhem" : "V trhu"}</div>
                </div>
              </div>
              <div style={{ fontSize: 11, color: "#888", marginBottom: 16, background: "#F0F4FF", borderRadius: 10, padding: "10px 14px" }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {[
              ...
              { id: "ai", icon: "✦", label: "AI Smart Pricing", desc: "Cena se dynamicky upravuje podle počtu zobrazení a zájmu. Trustera optimalizuje prodej automaticky." },
            ].map(opt => (
              <div key={opt.id} onClick={() => setForm(f => ({ ...f, pricingMode: opt.id }))} style={{ padding: "14px 16px", borderRadius: 12, border: "1.5px solid " + (form.pricingMode === opt.id ? "#0F0F0F" : "#E2E8F4"), background: form.pricingMode === opt.id ? "#F0F4FF" : "#fff", cursor: "pointer", display: "flex", gap: 14, alignItems: "flex-start" }}>
                <div style={{ width: 36, height: 36, borderRadius: 10, background: form.pricingMode === opt.id ? "#0F0F0F" : "#EEF2FB", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, flexShrink: 0 }}>{opt.icon}</div>
                <div style={{ flex: 1 }}>
                ...
                <div style={{ width: 20, height: 20, borderRadius: "50%", border: "2px solid " + (form.pricingMode === opt.id ? "#0F0F0F" : "#ccc"), background: form.pricingMode === opt.id ? "#0F0F0F" : "#fff", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
      ...
      <Card style={{ padding: 24, marginBottom: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          {[{ key: "hasGarage", label: "🚗 Garáž / parkovací místo" }, { key: "hasBalcony", label: "🏙 Balkón nebo terasa" }, { key: "hasGarden", label: "🌿 Zahrada" }, { key: "hasPool", label: "🏊 Bazén" }].map(item => (
            <div key={item.key} onClick={setFBool(item.key)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", background: form[item.key] ? "#f0fdf4" : "#F0F4FF", borderRadius: 11, border: "1.5px solid " + (form[item.key] ? "#6ee7b7" : "#E2E8F4"), cursor: "pointer" }}>
      ...
      <Card style={{ padding: 24, marginBottom: 24 }}>
        ...
        <textarea value={form.description} onChange={setF("description")} rows={5} placeholder="Popište nemovitost — co ji dělá výjimečnou, co je v okolí, stav…" style={{ width: "100%", padding: "11px 13px", borderRadius: 11, border: "1.5px solid #E2E8F4", fontSize: 13, fontFamily: "'Inter', sans-serif", resize: "vertical", boxSizing: "border-box" }} />
      </Card>
  ...
  return (
    <div>
      <div style={{ background: "#f0fdf4", border: "1px solid #6ee7b7", borderRadius: 12, padding: "14px 18px", marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <button onClick={() => setShowModal(true)} style={{ background: "none", border: "1.5px solid #6ee7b7", borderRadius: 9, padding: "7px 14px", fontSize: 12, fontWeight: 600, color: "#059669", cursor: "pointer", fontFamily: "'Inter', sans-serif", whiteSpace: "nowrap" }}>
          Zobrazit potvrzení →
        </button>
      </div>
      {showModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }} onClick={() => setShowModal(false)}>
          <div style={{ background: "#fff", borderRadius: 20, padding: 32, maxWidth: 500, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }} onClick={e => e.stopPropagation()}>
            <div style={{ textAlign: "center", marginBottom: 24, paddingBottom: 20, borderBottom: "2px solid #0F0F0F" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 10 }}>
                <div style={{ width: 32, height: 32, background: "#0F0F0F", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ color: "#1B4FD8", fontWeight: 900, fontSize: 14 }}>V</span></div>
                <span style={{ fontWeight: 800, fontSize: 16, fontFamily: "'Playfair Display', serif" }}>Trustera Legal</span>
              </div>
            ...
            {[
              ...
              <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "9px 0", borderBottom: "1px solid #F0F4FF", fontSize: 13 }}>
            ...
            <button onClick={() => setShowModal(false)} style={{ marginTop: 16, width: "100%", padding: "12px", borderRadius: 10, border: "none", background: "#0F0F0F", color: "#fff", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>Zavřít</button>
          </div>
        </div>
      )}
    </div>
...
const ViewingFeedback = ({ viewing, onSubmitted }) => {
  ...
  return (
    <div style={{ background: "#EFF6FF", border: "1px solid #BFDBFE", borderRadius: 12, padding: "16px 18px", marginTop: 12 }}>
      <div style={{ fontSize: 12, fontWeight: 700, color: "#1E3A8A", marginBottom: 12 }}>💬 Jak proběhla prohlídka? Sdělte nám svůj dojem</div>
      <div style={{ marginBottom: 14 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#1E3A8A", marginBottom: 8 }}>CELKOVÝ DOJEM</div>
        <div style={{ display: "flex", gap: 6 }}>
          {[1, 2, 3, 4, 5].map(s => (
          ...
          {stars > 0 && <span style={{ fontSize: 12, color: "#1E3A8A", fontWeight: 600, alignSelf: "center", marginLeft: 4 }}>{["", "Velmi špatná", "Špatná", "Průměrná", "Dobrá", "Výborná"][stars]}</span>}
        </div>
      </div>
      <div style={{ marginBottom: 14 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#1E3A8A", marginBottom: 6 }}>OČEKÁVÁNÍ CENY</div>
        <select value={priceExpect} onChange={e => setPriceExpect(e.target.value)} style={{ width: "100%", padding: "9px 12px", borderRadius: 9, border: "1.5px solid #BFDBFE", fontSize: 13, fontFamily: "'Inter', sans-serif", background: "#fff" }}>
      ...
      <div style={{ marginBottom: 12 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#1E3A8A", marginBottom: 6 }}>POZNÁMKA (nepovinné)</div>
        <textarea value={note} onChange={e => setNote(e.target.value)} rows={2} placeholder="Co se vám líbilo nebo nelíbilo?" style={{ width: "100%", padding: "9px 12px", borderRadius: 9, border: "1.5px solid #BFDBFE", fontSize: 13, fontFamily: "'Inter', sans-serif", resize: "none", boxSizing: "border-box" }} />
      </div>
      <div style={{ display: "flex", gap: 8 }}>
        <Btn small disabled={!stars || !priceExpect} onClick={submit}>Odeslat zpětnou vazbu</Btn>
        <button onClick={() => onSubmitted && onSubmitted()} style={{ background: "none", border: "none", fontSize: 12, color: "#aaa", cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>Přeskočit</button>
      </div>
    </div>
  );
};
      ...
      <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: "flex", gap: 10, fontSize: 12, marginBottom: 12, flexWrap: "wrap" }}>
            <span style={{ background: "#F0F4FF", padding: "4px 10px", borderRadius: 7, fontWeight: 600 }}>🕐 {v.time}</span>
            {v.date && <span style={{ background: "#F0F4FF", padding: "4px 10px", borderRadius: 7, fontWeight: 600 }}>📅 {v.date}</span>}
            <span style={{ background: "#F0F4FF", padding: "4px 10px", borderRadius: 7, fontWeight: 600 }}>👤 {v.buyer}</span>
          </div>
          {status !== "done" ? (
            <div style={{ display: "flex", gap: 8 }}>
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "28px 24px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <div style={{ textAlign: "center", background: "#fff", border: "1px solid #E8EDF8", borderRadius: 14, padding: "12px 20px" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
                    {[{ icon: "👁", label: "Zobrazení", v: lead.viewed, color: "#3b82f6" }, { icon: "📅", label: "Prohlídky", v: lead.viewings, color: "#d97706" }, { icon: "📋", label: "Nabídky", v: lead.offers, color: "#7c3aed" }].map(s => (
                      <div key={s.label} style={{ background: "#F0F4FF", borderRadius: 9, padding: "6px 12px", display: "flex", alignItems: "center", gap: 6 }}>
...
const AdminDash = () => {
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 24px" }}>
          ...
          <div style={{ background: "#fff", borderRadius: 20, padding: 32, maxWidth: 560, width: "100%", maxHeight: "85vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 24 }}>
              {[["Kupující", selectedTx.buyer], ["Prodávající", selectedTx.seller || "SLR-4482"], ["Nabídnutá cena", selectedTx.offered ? fmtFull(selectedTx.offered) : "–"], ["Vyvolávací cena", selectedTx.asking ? fmtFull(selectedTx.asking) : "–"], ["Financování", selectedTx.financing || "–"], ["Stav", selectedTx.status === "accepted" ? "✓ Přijata" : "⏳ Čeká"]].map(([k, v]) => (
                <div key={k} style={{ background: "#F0F4FF", borderRadius: 10, padding: "10px 14px" }}>
            ...
            <div style={{ position: "relative" }}>
              <div style={{ position: "absolute", left: 15, top: 0, bottom: 0, width: 2, background: "#EEF2FB" }} />
              {TX_TIMELINE.map((t, i) => (
                <div key={i} style={{ display: "flex", gap: 16, marginBottom: 16, position: "relative" }}>
                  <div style={{ width: 30, height: 30, borderRadius: "50%", background: t.status === "done" ? "#059669" : t.status === "active" ? "#0F0F0F" : "#E2E8F4", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, flexShrink: 0, zIndex: 1, border: t.status === "active" ? "3px solid #1B4FD8" : "none" }}>
                  ...
                  <div style={{ paddingTop: 4 }}>
                    <div style={{ fontWeight: t.status === "active" ? 700 : 500, fontSize: 13, color: t.status === "pending" ? "#bbb" : "#0F0F0F" }}>{t.event}</div>
                    <div style={{ fontSize: 11, color: "#bbb", marginTop: 2 }}>{t.date}</div>
                  </div>
                </div>
              ))}
            </div>
            <div style={{ borderTop: "1px solid #EEF2FB", paddingTop: 16, marginTop: 8 }}>
              <div style={{ fontSize: 12, color: "#888", marginBottom: 8 }}>Odpovědný advokát</div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div><div style={{ fontWeight: 700 }}>Mgr. Jana Horáčková</div><div style={{ fontSize: 12, color: "#888" }}>Trustera Legal, s.r.o.</div></div>
      ...
      <p style={{ color: "#888", fontSize: 13, marginBottom: 24 }}>Správa a monitoring Trustera</p>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 22, borderBottom: "1px solid #E2E8F4", overflowX: "auto" }}>
        {[
          ...
          <button key={t.id} onClick={() => setTab(t.id)} style={{ padding: "9px 18px", background: "none", border: "none", borderBottom: tab === t.id ? "2px solid #0F0F0F" : "2px solid transparent", color: tab === t.id ? "#0F0F0F" : "#888", fontWeight: tab === t.id ? 700 : 500, fontSize: 13, cursor: "pointer", fontFamily: "'Inter', sans-serif", marginBottom: -1, whiteSpace: "nowrap" }}>{t.label}</button>
        ))}
      </div>
      ...
      {tab === "overview" && (
        <div>
          ...
          <div style={{ background: "linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 100%)", borderRadius: 16, padding: 24, marginBottom: 20 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
              <div>
                <div style={{ color: "#1B4FD8", fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", marginBottom: 4 }}>UZAVŘENÁ TRANSAKCE</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 16 }}>
              {[
                ...
                { label: "PROVIZE Trustera (2%)", value: fmtFull(390000), color: "#1B4FD8" },
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => showToast("✓ Vyúčtování odesláno oběma stranám")} style={{ background: "#1B4FD8", color: "#0F0F0F", border: "none", borderRadius: 9, padding: "9px 18px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>
                Odeslat vyúčtování →
              </button>
              <button onClick={() => window.print()} style={{ background: "rgba(255,255,255,0.1)", color: "#fff", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 9, padding: "9px 18px", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>
                Tisknout
              </button>
            </div>
          </div>
      ...
      {tab === "viewings" && (
        <div>
          ...
          {ADMIN_VIEWINGS.filter(v => v.date === "Dnes").map(v => (
            <Card key={v.id} style={{ padding: 16, marginBottom: 10, border: "2px solid #BFDBFE" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                      ...
                      <div style={{ color: "#E2E8F4", fontSize: 20 }}>vs</div>
                      <div>
          ...
          {listings.filter(l => listingStatuses[l.id] === "pending").length > 0 && (
            <div style={{ marginBottom: 24 }}>
              ...
              {listings.filter(l => listingStatuses[l.id] === "pending").map(l => (
                <Card key={l.id} style={{ padding: 20, marginBottom: 12, border: "2px solid #BFDBFE" }}>
                  <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
                    <img src={l.img} alt={l.title} style={{ width: 80, height: 60, objectFit: "cover", borderRadius: 10, flexShrink: 0 }} />
                    <div style={{ flex: 1 }}>
                      ...
                      {showReject[l.id] && (
                        <div style={{ marginTop: 10 }}>
                          <input value={rejectReason[l.id] || ""} onChange={e => setRejectReason(s => ({ ...s, [l.id]: e.target.value }))} placeholder="Důvod zamítnutí…" style={{ width: "100%", padding: "9px 12px", borderRadius: 9, border: "1.5px solid #fca5a5", fontSize: 13, fontFamily: "'Inter', sans-serif", boxSizing: "border-box", marginBottom: 8 }} />
...
// ─── APP ──────────────────────────────────────────────────────────────────────
const ProtectedPage = ({ user, roles, setPage, children }) => {
  useEffect(() => {
    if (!user || (roles && !roles.includes(user.role))) setPage("login");
  }, [user, roles, setPage]);
  if (!user || (roles && !roles.includes(user.role))) {
    return <div style={{ maxWidth: 620, margin: "80px auto", padding: 24, textAlign: "center" }}>
      <div style={{ width: 48, height: 48, margin: "0 auto 16px", borderRadius: 14, background: "var(--brand-soft)", display: "grid", placeItems: "center", fontSize: 22 }}>🔒</div>
      <h1 style={{ ...D, fontSize: 28, marginBottom: 8 }}>Tato část vyžaduje přihlášení</h1>
      <p style={{ color: "var(--muted)" }}>Pro ochranu účtu a dokumentů se nejprve přihlaste.</p>
    </div>;
  }
  return children;
};
...
export default function App() {
  ...
  const renderPage = () => {
    ...
    if (page === "seller-dash") return <ProtectedPage user={user} roles={["SELLER"]} setPage={setPage}><SellerDash setPage={setPage} user={user} /></ProtectedPage>;
    if (page === "buyer-dash") return <ProtectedPage user={user} roles={["BUYER"]} setPage={setPage}><BuyerDash setPage={setPage} user={user} /></ProtectedPage>;
    if (page === "admin-dash") return <ProtectedPage user={user} roles={["ADMIN"]} setPage={setPage}><AdminDash setPage={setPage} /></ProtectedPage>;
    if (page === "agent-dash") return <ProtectedPage user={user} roles={["AGENT"]} setPage={setPage}><AgentDash setPage={setPage} user={user} /></ProtectedPage>;
    if (page === "new-listing") return <ProtectedPage user={user} roles={["SELLER"]} setPage={setPage}><NewListingPage setPage={setPage} user={user} /></ProtectedPage>;
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", background: "var(--canvas)", minHeight: "100vh", color: "var(--ink)" }}>
